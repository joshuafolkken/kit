...
**A fifth field ranks the point-of-use candidates, and until [#1871](https://github.com/joshuafolkken/kit/issues/1871) that ranking was a hand calculation — `documents`.** It carries one row per instruction document the run read at its entry (`.claude/…`, `prompts/…`, or a root agent doc such as `CLAUDE.md`), largest carry cost first: each row's `tokens` is what the document stacked, `carried_requests` is how many billed requests re-read it from cache after it was read, and `cost_usd` is that carry priced at the cache-read rate. **This is exactly the reading a point-of-use decision needs** — moving a document from "read at the entry" to "read at the point of use" removes its `carried_requests`, so the top rows are the candidates worth converting and their `cost_usd` is the saving. #1856 chose its target (`chain-rule.md`) from a hand calculation over the transcript; read the top of this table instead, and rank a conversion row in step 3 on its own `cost_usd`, the same way a phase row is ranked on `phase_costs`. **Read `is_measured` before quoting a row**: a `false` is a document whose `Read` was found but whose result the transcript did not carry, so its tokens and cost are **unmeasured rather than zero** — the same withheld-is-not-zero distinction the rest of this report draws — and it is reported rather than dropped. **The field is present on the `--issue` and single-session scopes and absent on `--all`**, which reads no transcript for it; an empty `rows` on a measured scope means no entry-read document was found, which is a real answer for a short run.
