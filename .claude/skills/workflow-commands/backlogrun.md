...
`sonnet` and reviewer `opus`; OpenAI uses `gpt-5.6-sol`, with role efforts
`high`/`medium`/`high` for both. Role-specific environment overrides are resolved before launch;
