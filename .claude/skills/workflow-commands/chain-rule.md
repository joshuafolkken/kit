...
2. **Severity check** — Count high/medium findings. If ≥1 → fix in place and re-run `/code-review medium`. Nothing is committed yet, so the loop costs no commit or CI run. Do NOT call `followup --merge` yet. **Stop at two rounds:** after the second, file every remaining non-High finding as a follow-up Issue referencing this one and continue the pipeline. A confirmed High is the only thing that blocks past the cap, and one still standing after the second round means the change is not ready — send a `confirmation` Telegram and put the scope back to the user instead of starting a third round.
