...
| One or more High / Medium findings      | High and/or Medium | Fix in place, then run the **second-round verification pass** — `/code-review` at the level `pnpm josh review:level` prints, scoped to the **fix delta** and asking whether each first-round finding closed, **not** a second adversarial read of `git diff main` (`prompts/review.md` → "The second round is a verification pass, not a second full review") — **at most two reviews in total** (`prompts/review.md` → "Review round cap"). Nothing is committed yet, so a round costs no commit, push, or CI run. Do NOT report narratively and wait. |
