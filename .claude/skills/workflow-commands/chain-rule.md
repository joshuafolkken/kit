...
| One or more High / Medium findings      | High and/or Medium | Fix in place and re-run `/code-review` at the level `pnpm josh review:level` prints on `git diff main` — **at most two reviews in total** (`prompts/review.md` → "Review round cap"). Nothing is committed yet, so a round costs no commit, push, or CI run. Do NOT report narratively and wait. |
