...
the session. **300,000 is a temporary experiment rather than a settled number** (joshuafolkken/kit#1775) — the figure it replaces, the retreat and its review point are all in that same single source. **The resume command is the rest of the queue**, `queue #<next> #<after> …` from the
