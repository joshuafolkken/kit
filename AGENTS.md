...
#### `halfrun` — Implement + PR, stop before merge (for manual verification)

`halfrun` sits between `kickoff` (plan only) and `fullrun` (full execution with auto-merge). It runs the full implementation pipeline — including commit, push, PR creation, and the fresh-context review — but **stops before merge** so the user can manually verify the change (typically by exercising the UI in a browser) while the PR is still OPEN. Use `halfrun` when the change needs human eyes before shipping.

- `halfrun #<N>`: Read Issue #N → **normalize the title** (same as `fullrun`) → **add `in-progress` label** (create if missing: `gh label create "in-progress" --color "#0075ca" --description "Work is actively in progress" 2>/dev/null || true`, then `gh issue edit <N> --add-label "in-progress"`) → post the agreed plan only if the Issue body is blank → run `git switch main && git pull`, then `josh latest` (verify the overrides are unchanged in **both** locations — `pnpm-workspace.yaml` `overrides:` and `package.json` `pnpm.overrides` (`git diff -- pnpm-workspace.yaml package.json`; an empty `pnpm.overrides` is not evidence of absence) — and `devEngines` changed only by the expected `josh latest` pnpm bump; restore if otherwise modified) → implement → run the **verification gate** (refactor → `pnpm josh lint` → `pnpm exec tsc --noEmit` → `pnpm josh cspell:dot` → `pnpm josh test:unit`; the review step runs after the PR exists, not here) → `pnpm josh bump minor` → `pnpm josh git -y "<title> #<N>"` (creates the PR) → launch the `code-reviewer` subagent on the PR diff per "Review step execution" below, fixing high/medium findings in place (commit + push each round) until clean, posting each round's review markdown as its own PR comment as that round completes → send a `confirmation` Telegram with the single resume command in the body → **stop with the PR OPEN**. Plan comments MUST be in English. The `confirmation` Telegram body MUST include the exact resume command: `pnpm josh notify --task-type confirmation --issue-url "<issue-url>" --body=$'halfrun ready for manual verification — PR is open\nNext: pnpm josh followup "<title> #<N>" --merge --notify-message "Implemented <title>\\nCause: ...\\nFix: ...\\nResult: ...\\n\\nDetails:\\n- <change1>"'`. **Invoking `halfrun` authorizes commit, push, and PR creation, but NOT merge** — do not run `pnpm josh followup --merge` yourself; the user verifies the open PR and runs the resume command manually. If the user comes back with fixes, treat each round as: implement → re-run the verification gate → commit and push to the same PR → re-run the subagent review → post that round's review comment → send another `confirmation` Telegram → stop.
- `halfrun new` or `halfrun new "<title>"`: Shortcut that combines `kickoff new` + `halfrun #<N>`. Steps mirror `fullrun new` (1)–(8): (1) Derive an English title or use the provided title. (2) `gh issue create --title "<title>" --body "<body>"`. Capture `<N>`. (3) Add `in-progress` label. (4) Post the agreed plan in English. (5) `git stash` if the working tree has changes. (6) `git switch main && git pull`. (7) `josh latest` — **mandatory**. Verify the overrides are unchanged in **both** locations — `pnpm-workspace.yaml` `overrides:` and `package.json` `pnpm.overrides` (`git diff -- pnpm-workspace.yaml package.json`; an empty `pnpm.overrides` is not evidence of absence) — and `devEngines` changed only by the expected `josh latest` pnpm bump; `git stash pop` if stashed. (8) Implement. (9) Run the verification gate (refactor / lint / tsc / cspell / test:unit). (10) `pnpm josh bump minor`. (11) `pnpm josh git -y "<title> #<N>"` (creates the PR). (12) Launch the `code-reviewer` subagent on the PR diff, fix high/medium findings (commit + push each round) until clean, posting each round's review markdown as its own PR comment as that round completes. (13) Send the `confirmation` Telegram (same body as `halfrun #<N>`) and **stop with the PR OPEN**. Do not run `pnpm josh followup --merge`.
...
2. For each issue `#<N>` in the supplied order:
   ...
   b. Execute the full `fullrun #<N>` flow: normalize title → add `in-progress` label → post plan if body is blank → implement → `pnpm josh bump minor` → `pnpm josh git -y "<title> #<N>"` → run `/review` skill (posting each round's review markdown as its own PR comment as that round completes) → `pnpm josh followup "<title> #<N>" --merge --notify-message "Implemented <title>\nCause: ...\nFix: ...\nResult: ...\n\nDetails:\n- ..."` (sends per-issue completion notification and merges, exactly as `fullrun` does) → `pnpm josh ms` (return to the default branch).
...
The workflow review step (`/review` in the flows above) runs **only after `pnpm josh git -y` has created the PR** — there is no pre-commit variant; `fullrun`, `halfrun`, and `queue` all review the same target, the PR branch's `git diff main...HEAD`. It is executed by launching the kit-distributed `code-reviewer` subagent (`.claude/agents/code-reviewer.md`) — a fresh context that did not implement the change, so the author's assumptions are not carried into the review. Pass it only the diff scope and the Issue title/goal; never the implementing session's reasoning or a summary of the changes. Its output follows the `prompts/review.md` template, so the chain rule and decision table below apply unchanged. **Post every review round as its own PR comment** (`gh pr comment <branch> --body-file <path>`), immediately after that round's review returns and **before** its findings are fixed. Head each comment `## Code review — round N`, numbered from 1 and continuing across a re-review after later commits. Determine `N` by counting the `## Code review — round` comments already on the PR (`gh pr view <branch> --json comments --jq '[.comments[].body | select(test("^## Code review — round"))] | length'`) and adding 1 — a resumed session has no memory of the last round, and restarting at 1 reintroduces the very "which comment is authoritative" ambiguity this rule removes. Each comment carries the **full** review markdown, never a delta against the previous round, so it stands on its own when read later; the final round states that the loop settled with zero high/medium findings, so the authoritative result is unambiguous. **Posting only the settled round is what this replaces**: it collapsed the whole loop into the last round's timestamp and discarded the findings that drove each fix commit, leaving a three-round review indistinguishable from a clean one-round one. The comments are authored by the invoking user's GitHub account, so the `followup` AI-reviewer blocker scan (which keys on `claude` / `coderabbitai` authors) never treats them as blockers — which is what makes posting a round that still carries high/medium findings safe. In AI tools without subagents (Gemini, Cursor), run the `prompts/review.md` checklist in a new session/context instead. See `prompts/collaboration-workflow.md` → 「レビュー工程はフレッシュコンテキストのサブエージェントで実行する」.
...
| --------------------------------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Clean — every category says `No issues` | None               | Post this round's review markdown as a PR comment, then immediately run `pnpm josh followup "<title> #<N>" --merge --notify-message "..."`                                                                            |
| Low findings only                       | Low                | Post this round's review markdown as a PR comment, then immediately run `pnpm josh followup --merge` (Low may be skipped with a one-line reason)                                                                      |
| One or more High / Medium findings      | High and/or Medium | **Post this round's full review markdown as a `## Code review — round N` PR comment first**, then fix in place, re-stage, commit, push, re-run `/review`, loop. Do NOT report narratively and wait. Do NOT merge yet. |
...
1. **Mode check** — Is this `/review` part of a `fullrun` / `fullrun new` / `queue` invocation? Decide by both signals: (a) the user's recent prompt contained one of those commands, AND (b) `pnpm josh git -y` has already been run in this session and a PR exists. **A `halfrun` invocation never satisfies (a)** — halfrun reaches PR creation and this review too, but it ends at the confirmation stop: post the PR comment, send the `confirmation` Telegram, and stop with the PR OPEN. If either signal is false → NOT fullrun mode; do NOT call `followup --merge`.
2. **Severity check** — Count high/medium findings. If ≥1 → post this round's review comment first, then fix, re-stage, commit, push, re-run `/review`. Do NOT call `followup --merge` yet.
