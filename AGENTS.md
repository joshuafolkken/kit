...
2. For each issue `#<N>` in the supplied order:
   ...
   b. Execute the full `fullrun #<N>` flow: normalize title → post plan if body is blank → implement → `pnpm josh bump minor` → `pnpm josh git -y "<title> #<N>"` → `pnpm josh followup "<title> #<N>" --merge --notify-message "Implemented <title>:\n- ..."` (sends per-issue completion notification and merges, exactly as `fullrun` does).
