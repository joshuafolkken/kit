...
- **Third-party repositories are Tier C.** Decide first-party vs third-party mechanically by owner equality (`gh api repos/{owner}/{repo} --jq .owner.login`). Every write to a tracker we do not own needs explicit current-turn instruction — a correct diagnosis is not authorization to publish. `prompts/collaboration-workflow/upstream-interrupt.md`.
...
| `diag [fullrun \| backlogrun \| #N]` | Measure where a run's time went and rank what to cut — analysis only                                                                                                  | `diag` skill                                                     |
