...
1. **Mode check** — Is this `/review` part of a `fullrun` / `fullrun new` / `queue` invocation? Decide by both signals: (a) the user's recent prompt contained one of those commands, AND (b) `pnpm josh git -y` has already been run in this session and a PR exists for the current branch. **A `halfrun` invocation never satisfies (a)** — halfrun reaches PR creation and this review too, but it ends at the confirmation stop: post the PR comment, send the `confirmation` Telegram, and stop with the PR OPEN. If either signal is false → you are NOT in fullrun mode; do NOT call `followup --merge`.
