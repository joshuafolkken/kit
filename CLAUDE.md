...
Run the full verification set **in order**; do not skip, reorder, or report completion on a failed or skipped step without the user agreeing. Inside a `fullrun` / `queue` / `epicrun` the gate runs beside the review and the merge chain — full procedure in the `workflow-commands` skill → §2 and `prompts/review.md`.
...
| `queue #N1 #N2 …`                 | `fullrun` for each Issue in the given order, stopping at the first failure                                                  | `workflow-commands` skill + `queue.md` + the `fullrun` set                           |
...
| `backlogrun`                      | Run the opted-in backlog unattended — dependency order, lanes, no epic named                                                | `workflow-commands` skill + `backlogrun.md` + the `epicrun` set                      |
...
**A session cut inside a declared budget is not a new invocation** — a `backlogrun` or `queue` cut and resumed is the one invocation a person typed, and it continues rather than waiting to be retyped. A run with no budget left, or none begun, has nothing to carry, so the bullet above still stands. **`backlogrun` and `queue` alone**: an `epicrun` or `fullrun` cut still waits for the keyword. Single source: `backlogrun.md` → "The session cut is inside the invocation"; a queue's invocation is pinned to the issue list that was typed, per `queue.md` → "The session boundary".
