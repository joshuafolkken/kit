...
Every `fullrun` / `fullrun new` invocation uses `pnpm git:followup --merge`, which handles the full sequence internally: wait for CI → verify AI review findings → send completion notification → merge. The user does **not** need to add a keyword. Invoking `fullrun` is itself the explicit authorization to merge.

```bash
pnpm git:followup "<title> #<N>" --merge --notify-message "..."
```

- **AI review findings are checked automatically.** `pnpm git:followup --merge` scans for CodeRabbit / Claude Review / SonarQube findings before merging. If blockers are found, it sends a `confirmation` Telegram and exits non-zero — fix the findings and re-run `pnpm git:followup --merge`. **Green CI is not authorization to merge while AI review findings are open.**
...
- **If the user wants to skip the merge step**, use `kickoff` (plan-only) or explicitly say "do not merge" / "do not auto-merge" in the same turn. In that case, omit `--merge` from `pnpm git:followup`. Outside a `fullrun` invocation, never run `gh pr merge` on your own.
...
- Applies to the initial PR and every follow-up commit (CodeRabbit fixes, re-review iterations, merges from main, etc.) — re-run `pnpm git:followup "<title> #<N>" --merge --notify-message "Implemented <title>:\n- <change1>\n- <change2>\n..."` each time you want to notify completion (notification is sent right before the merge).
