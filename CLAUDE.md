...
- Resolve **all high and medium findings** before committing. Low findings that do not reach the user may be skipped with a one-line reason — the same disposition the round cap's branch 3 extends past the second round, so the two documents agree on what happens to a Low.
