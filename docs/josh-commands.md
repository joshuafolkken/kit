...
- **The call is bundleable.** Every mutation word is outside the allow-list — `pnpm`, `git`, `node` and the `gh` write flags are all mutation words — so no `pnpm josh` command, no commit and no Issue write is reachable from here. **Writing is no longer a second question** ([#1762](https://github.com/joshuafolkken/kit/issues/1762)): `Edit` and an in-place `sed` are candidates, and what keeps the unsafe ones out is the shared-target test above rather than their kind — `Write` alone is named out, per the paragraph above. The question is asked **before the transcript is read**, so a `pnpm josh` call costs the hook's own start and nothing more. The read-only form of it survives as `is_read_only_call`, which `josh investigation:guard` asks — that guard counts reading, so a line that also writes is not one for it to stop.
