...
Before invoking corepack, `latest:corepack` also **age-gates** the candidate pnpm: it resolves the version behind `latest-<major>` and its publish time from the npm registry, reads the `minimum-release-age` window from `.npmrc`, and **holds** the bump (printing a `⚠ Held pnpm bump …` notice, leaving `package.json` untouched) when the release is still inside that window. This prevents kit from adopting a just-published pnpm that a consumer's CI cannot yet install through `pnpm/action-setup`'s self-installer under safe-chain — the self-installer crashes on the degraded (`dist.integrity` undefined) manifest that safe-chain returns for a too-new version. The gate is age-driven, so it lifts automatically once the release ages past the window; there is no hardcoded pin to maintain. Any failure to determine the age (missing `.npmrc`, registry unreachable, unexpected shape) fails open so a transient hiccup never freezes pnpm permanently.
