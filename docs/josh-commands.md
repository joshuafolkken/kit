...
The reason names the list beside each path, because that is the half a reader cannot derive. The changed paths come from the same helper `josh review:level` and `josh review:brief` read, so all answer from one definition of what changed.
