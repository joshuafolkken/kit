...
**And the invocation it wakes with is held to the shape of a `backlogrun`.** That is the one thing this supervisor exists to continue, so an allowlist is available here where it would not be for a general-purpose launcher — and it is a real narrowing rather than a formality: without it the supervisor would launch a session with whatever text the carry record happened to hold, which is the record's integrity standing in for a check nobody performs.
