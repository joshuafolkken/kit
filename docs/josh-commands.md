...
**It is the ledger's only commit path** ([#1756](https://github.com/joshuafolkken/kit/issues/1756)). `pnpm josh git` excludes `docs/observations.md` from what it stages, so an append made in the primary checkout can never ride into a pull request about something else — and, with that exclusion in place, has no other route to the default branch either. This command stages that one path, commits it on a branch named `observations/<YYYY-MM-DD-HHmm>`, opens a docs-only pull request with no `closes #N`, waits on the same required checks every other pull request waits on, merges it and returns the checkout to the default branch.
