...
**The matcher names the two edit tools beside `Bash` since [#1762](https://github.com/joshuafolkken/kit/issues/1762).** It named `Bash` alone for as long as the script answered "not a candidate" to every write, which made a wider matcher a process that could only ever say "allow" — that cost argument was about _widening the matcher and nothing else_, and it still holds in that form. What changed is the predicate underneath it: a write is refusable now, so the matcher reaches a question that has an answer. **The reason to pay for it is reach.** Read over 19 runs, 261 of 1,737 round trips were recoverable by batching and **184 of them (70.5%) were writes** — 158 `Edit` alone — which `Bash` cannot reach however large its share of the calls.

**Refusing a write is safe because of the target test, not because of a blanket exclusion.** Claude Code denies one call of a turn and runs the rest, so a refused `Edit` leaves its siblings applied and itself not. The case where that costs something is an edit naming a file the run is already rewriting — there the reissue meets text that has moved. The guard therefore **withholds the refusal whenever the call in hand shares a target with the sequence behind it**, writes included, which is where it departs from the sequence builder: that one exempts a write following a write on purpose, so such a stretch forms a sequence at all ([#1509](https://github.com/joshuafolkken/kit/issues/1509)). What is left refusable is an edit to a file nothing in the sequence touches, whose reissue lands on exactly the text it was written against.

**The half the target test cannot close is the turn being interrupted**, for the reason the module states: no field in the transcript says what else that turn issued, so three edits to one file batched into one turn can have their first refused while the other two apply. **The bound is that it surfaces rather than corrupts** — the reissued edit carries the `old_string` it was written with, so it either matches or fails to match and is reported. The residual is one round trip in the ordinary case and a failed edit to redo in this one, which is the false-positive price already accepted for reads.
...
- **The call is bundleable.** Every mutation word is outside the allow-list — `pnpm`, `git`, `node` and the `gh` write flags are all mutation words — so no `pnpm josh` command, no commit and no Issue write is reachable from here. **Writing is no longer a second question** ([#1762](https://github.com/joshuafolkken/kit/issues/1762)): `Edit`, `Write` and an in-place `sed` are candidates, and what keeps the unsafe ones out is the shared-target test above rather than their kind. The question is asked **before the transcript is read**, so a `pnpm josh` call costs the hook's own start and nothing more. The read-only form of it survives as `is_read_only_call`, which `josh investigation:guard` asks — that guard counts reading, so a line that also writes is not one for it to stop.
