...
One consequence is worth naming because it is deliberate. `epic:audit` still exempts a pair of children in two different repositories from its order-contradiction check. That exemption existed only because such an order could not be recorded at all, and the premise is gone — but lifting it turns an **error** red on epics that are green today, which would stop every `epicrun` on them at its first step. Whether to lift it is a decision about audit strictness rather than about reading the relation, and it is [#1128](https://github.com/joshuafolkken/kit/issues/1128)'s to make.
