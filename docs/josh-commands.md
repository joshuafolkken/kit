...
The answer (`managed` or `clean`) goes to stdout and the reason to stderr, so a caller can read one without parsing the other. Exit status is `0` for both answers — this reports, it does not gate; the gate is `pnpm josh followup`, which asks the same question and stops on `managed`.

**A command rather than an instruction, for the reason `josh review:level` is one.** The confirmation stop it feeds used to be a paragraph telling the run to compare `git diff main...HEAD` against three arrays in `scripts/init/init-logic.ts` by eye. Measured over one epic's children on 2026-09-08, three pull requests changed a distributed file and **two merged without the comparison being made at all**.
...
pnpm josh followup "PR title #N" --managed-config-ignore-reason "distributing this is the point of the PR"
pnpm josh followup "PR title #N" --no-merge
```

**A change to a file `josh sync` distributes stops the merge, and it is the command that decides so** ([#1578](https://github.com/joshuafolkken/kit/issues/1578)). Before the CI wait, `followup` matches the branch diff against the three distribution lists — the same question [`josh sync:scope`](#josh-syncscope) answers — sends a `confirmation` Telegram and exits non-zero on a hit. It runs first precisely because the answer needs no check result: a run that is going to stop stops in seconds instead of after the full wait.

Until then the stop was a paragraph in the workflow skill telling the run to compare the diff against three arrays by eye, and over one epic's children **two of the three pull requests that changed a distributed file merged without the comparison being made**. `--managed-config-ignore-reason "<reason>"` is the way past it: the reason and the claimed paths are posted to the pull request as a comment, and an audit note rides into the completion notification, so a bypassed run never reports like one that touched nothing distributed. A blank or whitespace-only reason is not a reason and does not open the gate.
...
| Field                      | What it says                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `at <YYYY-MM-DDTHH:MM>Z`   | When the observation was taken, with its date, in UTC. It leads the line because it is the field that says whether anything after it is still about now, and it is **beside** the elapsed figures rather than instead of them: a relative figure means something only while the reports keep coming, and a session suspended overnight read its own `quiet 11m` as if no time had passed ([#1560](https://github.com/joshuafolkken/kit/issues/1560)). The date is carried because that happened across a day boundary, and the zone is UTC — never the reader's — because the line is relayed to other machines and cloud sessions |
