...
**What the arm is worth, measured over 296 recorded sessions and 3,063 classified main-line reads.** Replaying both spellings of the rule over the same corpus: reads that reached the threshold and were let through fall from **1,124 (36.7% of every read)** to **910 (29.7%)**, and refusals rise from **186** to **400** — 214 reads the guard now speaks on and did not. **The two rows that must not move do not move**: reads of a file the run itself edits stay at 827 and reads below the threshold at 926, so nothing the rule leaves in the main line by design is touched. The 910 that remain are the design rather than a residue — each accumulation still lets the threshold minus one through between refusals, which is what keeps a false positive costing one round trip instead of wedging the run. `pnpm josh time`'s `Investigation reads:` block prints those four figures for one run, from this guard's own predicates.
