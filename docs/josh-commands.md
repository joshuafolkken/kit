...
- **The command must not need node flags of its own.** `doctor`, `latest:scope`, `followup` and `notify` each pass `--env-file`, which has to be in force before the script's first line; those four keep a process of their own. Each runs at most a few times per run, so none of them is where the cost accumulated.
...
**That single re-run is what an implementation loop is meant to use, and the whole gate is not.** A workflow run starts one gate, beside the review ([#1242](https://github.com/joshuafolkken/kit/issues/1242)), and re-runs a check by name until that point — ten whole gates cost 8.2 minutes of a 49.1-minute run, six of them before the review had started and every one of those answered by one check ([#1246](https://github.com/joshuafolkken/kit/issues/1246)). The rule itself is `.claude/skills/workflow-commands/chain-rule.md` → "The gate runs beside this review, not in front of it"; what this command contributes is the header line that names the one command to repeat. **Two of the checks that loop repeats are scoped**: [`josh lint:related`](#josh-lintrelated) checks only the changed files ([#1298](https://github.com/joshuafolkken/kit/issues/1298)) and [`josh test:related`](#josh-testrelated) runs only the tests related to them ([#1257](https://github.com/joshuafolkken/kit/issues/1257)); the gate itself keeps running `josh lint` and `josh test:unit` over everything.
...
**The matcher names `Read` as well since [#1798](https://github.com/joshuafolkken/kit/issues/1798), and there the predicate never had to move.** `Read` has answered "candidate" for as long as the test has been whether a call could have gone out beside another, so the wiring alone was keeping the refusal off it. Measured on `fullrun #1783`, **46 of the main line's 76 requests issued a single call**, in **7 runs of three or more** — and only **2** refusals fired, because 4 of those 7 held a `Read`, a `Write` or an `Agent` the matcher never handed over. **The counting was never the part that was wrong**: a single-call turn extends the run whenever its call is bundleable, which `Read`, `Glob`, `Grep` and `Write` all are, so what breaks a run is a call that could not have been batched at all — a delegation, whose result the next call needs. `Write` is named in the matcher too now, but for a notice rather than a refusal — the reason is further below.

**`Read` is now claimed by two refusing hooks, and that overlap is accepted rather than mediated.** `josh investigation:guard` matches `Read|Bash`, so since [#1798](https://github.com/joshuafolkken/kit/issues/1798) both guards can deny the same `Read`. **The overlap is not new** — it has held on `Bash` for as long as the investigation guard has existed — and it is left alone deliberately: Claude Code delivers **both** refusals to the caller, so neither rule goes unsaid, and the two ask for actions that agree rather than conflict (reissue the call beside its siblings; send the reading to a unit of its own). The stand-aside `delivered-rules.ts` carries is for a different problem: its rows deliver **once per run**, so a delivery lost to another hook is lost for good. **Neither guard here is in that position** — `batch:guard`'s record is one refusal per sequence and re-arms as soon as a new run of single-call turns begins, and `investigation:guard`'s is one per accumulation and re-arms after the next three unedited reads ([#1764](https://github.com/joshuafolkken/kit/issues/1764)) — so a collision costs at most a duplicated message, never a rule that goes unsaid. Making the investigation guard stand aside would trade that for a silenced threshold, which is the wrong direction.
...
**The half the target test cannot close is the turn being interrupted**, for the reason the module states: no field in the transcript says what else that turn issued, so three edits to one file batched into one turn can have their first refused while the other two apply. **The bound is that it surfaces rather than corrupts** — every refusable write is content-addressed, so a reissued `Edit` carries the `old_string` it was written with and a reissued in-place `sed` its pattern, and each either applies where it was meant to or fails to match and is reported. The residual is one round trip in the ordinary case and a failed edit to redo in this one, which is the false-positive price already accepted for reads.

**`Write` is the one write that is never refused, and that is what makes the bound above true rather than an exception to it.** A reissued `Write` is unconditional: it re-applies content composed before its turn's siblings ran, so it overwrites an applied edit with nothing raised anywhere — and a turn holding a `Write` and an `Edit` of one file is a shape a run produces (create it, then adjust it). A truncating shell redirect is wholesale too, but the colliding turn would have to hold two writers of one path, which is not a shape that occurs. So `is_guarded_call` answers "not a candidate for refusal" for it, and always will.

**But a run of single-call `Write` turns is notified since [#1848](https://github.com/joshuafolkken/kit/issues/1848), even though it is never refused.** Writes were the largest recoverable contributor the refusal could never reach, so the matcher now names `Write` and `is_notice_call` answers for it: a third consecutive single-call turn whose call is a write earns a **non-blocking notice** (`notice_envelope`, carrying no `permissionDecision`, so the write proceeds) instead of a refusal. The notice keeps a record of its own, distinct from the refusal's, so firing it never moves the instant a later read's or edit's refusal reads; it fires once per run of single-call turns, exactly as the refusal does. Naming `Write` in the matcher is worth the process start now that it reaches `is_notice_call`, a question with an answer — the same reasoning that admitted `Edit` and `Read`.

**What a call is stays the script's answer rather than the matcher's** — a matcher is settings a consumer can widen, and the guarantee has to hold whatever the wiring says. Inside `Bash` the script scans the **whole line**, not its leading word: a chain is labelled by its first segment, so `cat notes.md && sed -i '' s/a/b/ src/x.ts` reads as `cat` and a leading-word test would call it a read. `sed`, `tee` and `dd` anywhere in the line make it a write, as does a `>` anywhere in it. Both over-match — a read-only `sed`, a `->` inside a grep pattern — which allows a call that could have been refused, the direction every rule here leans.
...
- **The call is bundleable.** A commit, a push and an Issue write stay out — `git`, `node` and the `gh` write flags are mutation words, and `pnpm` leads the writing josh commands (`gate`, `followup`, `git`) past the allow-list too. **The read-only `pnpm josh` bookkeeping commands are the exception since [#1875](https://github.com/joshuafolkken/kit/issues/1875)**: `issue:state`, `release:scope` and the rest of `READ_JOSH_SUBCOMMANDS` (an alias resolved to its canonical name first) are bundleable, because they were the largest class of independent late-run calls the guard could never see — the leading `pnpm` had turned the read-only half away with the writing half, so `Bundling:` never counted them and the guard never refused them. A chained line still falls through, since the leading `pnpm` would hide a later mutation, and a subcommand that writes or sends (`run:progress --mark`, `notify`, `main:sync`) is left off the allow-list. **Writing is no longer a second question** ([#1762](https://github.com/joshuafolkken/kit/issues/1762)): `Edit` and an in-place `sed` are candidates, and what keeps the unsafe ones out is the shared-target test above rather than their kind — `Write` alone is named out, per the paragraph above. The question is asked **before the transcript is read**, so a non-candidate `pnpm josh` call costs the hook's own start and nothing more. The read-only form survives as `is_read_only_call`, which `josh investigation:guard` asks — that guard counts reading, and a bookkeeping call names no file target, so it is skipped there rather than counted toward the read threshold.
...
**So the counting is done off the transcript.** A delegation clears the pending set, a read adds its files to it, and a **file the run has edited within the window is never in it** ([#1840](https://github.com/joshuafolkken/kit/issues/1840)) — before the edit or after it, which is what makes the set mean "read and not edited" without asking the run to declare its intentions in advance, and what keeps [#1426](https://github.com/joshuafolkken/kit/issues/1426)'s rule that a file this run will edit is read in the main line. Nothing has to be remembered, so the second accumulation is indistinguishable from the first and the threshold necessarily fires again.

**The matcher names `Read` and `Bash`, and both halves are load-bearing.** In this repository the reading is split between them: run #1441 issued 5 `Read` calls against 10 `cat`, 16 `sed` and 1 `tail`, so a `Read`-only wiring would miss the idiom that carries most of the text. On the `Bash` side the guard refuses only a line that writes nothing — `is_read_only_call`, which treats `sed`, `tee`, `dd` and a `>` anywhere in the line as writing. That was the batching guard's own test until [#1762](https://github.com/joshuafolkken/kit/issues/1762) widened that one to cover writes; it kept its semantics here under a name that states them, because this guard counts _reading_ and a line that also writes is work rather than investigation. A `sed -n` read is therefore **counted and never refused**; the refusal lands on the next call that is unambiguously a read. What counts as reading is narrower than `time-bundle-call.ts`'s `READ_COMMANDS`: `bat`, `cat`, `head`, `less`, `more`, `nl`, `sed` and `tail` print a file, while `grep`, `ls` and `wc` report _about_ one without putting its text in the prompt.
...
**The harness's own session files are excluded too, and for that same reason rather than a second one** ([#1771](https://github.com/joshuafolkken/kit/issues/1771)). A backgrounded call's result reaches its run **only** as `tasks/<id>.output` under this session's temp tree, and a tool result too large to sit in the transcript reaches it **only** as a file the harness wrote under `.claude/projects/`; the scratchpad and the transcript are the same kind of thing. None of them is the Issue's subject — each is the run collecting the answer to an instruction it issued itself — and **no unit can be sent to read one**, which is the test both exclusions turn on: the remedy the refusal names does not exist for a file that belongs to this session alone, so the round trip buys nothing and the read is simply reissued. The match is anchored on what the harness **writes** — a file named `<id>.output` whose own parent is `tasks/`, inside a `claude-<uid>` temp tree; or anything under `.claude/projects`, which holds the transcripts and the persisted tool results and no subject code — **and** on the path lying outside the checkout, so a repository file is never exempted here whatever its own directories are called: `scripts/tasks/one.ts` and `.claude/settings.json` are counted exactly as before. **The scratchpad is deliberately not exempt**: a run may unpack or clone subject material into it, and exempting the session tree as a whole would have left those reads uncounted in silence — a false negative here prints nothing, so nothing would have shown it.

**Those two exclusions are the whole of what is not counted, and neither moves the threshold.** Counted: every other file, wherever it sits, read through `Read` or through one of the eight printing commands above. Not counted: the run's own instructions, and the harness's own session files. A call naming only excluded files is never refused, while a call naming **one** countable file is refused at the threshold however many excluded files it also names — which is why bundling a workflow procedure document into the same `cat` as a background call's output used to make the procedure document look refused. The refusal was the output file's, and a denied call takes its siblings with it.
...
**One refusal per accumulation, re-armed by a delegation, by another accumulation, or by the recorded refusal falling out of the window.** A run that reads on regardless is not refused a second time, so a false positive — three files it was about to edit — costs one round trip rather than wedging the run on the file it needs. **The second arm exists because the first one alone falls silent on a long run**: once the last delegation has scrolled out of the 256 KB tail there is no reset instant left to beat, and one stale stamp would disarm the rest of the session — on exactly the run lengths this was filed about. It is the same bound `josh batch:guard` documents for its own window, one extra refusal per window rather than silence, and it errs toward delegating. **The record is written before the refusal is made**: with nothing on record every later look at the same accumulation qualifies, so a hook that could not write one must not refuse. Every other failure allows the call too — a missing transcript, a payload that is not JSON, a tail caught mid-append. A hook firing inside a delegated unit is handed the _parent's_ transcript path and derives the fork's own file ([#1424](https://github.com/joshuafolkken/kit/issues/1424)), and since [#1840](https://github.com/joshuafolkken/kit/issues/1840) it does not refuse a unit's reading at all — a unit is already where this rule sends the reading, so the refusal asked for an action the unit could not take.

**The second arm above was published here before the code did any of it, and that gap was the defect** ([#1764](https://github.com/joshuafolkken/kit/issues/1764)). This page already said the record re-arms after the next three unedited reads; `is_rearmed` re-armed only on a delegation or on the window scrolling past, so a run that **ignored** its one refusal — read on and never delegated — was never spoken to again however many unedited files it went on to open. The one run the threshold exists for was the one run the guard fell silent on, which is the shipped-mechanism-not-working shape [#1262](https://github.com/joshuafolkken/kit/issues/1262) declared highest priority.

**What the arm is worth, measured over 296 recorded sessions and 3,064 classified main-line reads.** Replaying both spellings of the rule over the same corpus: reads that reached the threshold and were let through fall from **1,132 (36.9% of every read)** to **920 (30.0%)**, and refusals rise from **184** to **396** — 212 reads the guard now speaks on and did not. **The two rows that must not move do not move**: reads of a file the run itself edits stay at 827 and reads below the threshold at 921, so nothing the rule leaves in the main line by design is touched. The 920 that remain are the design rather than a residue — each accumulation still lets the threshold minus one through between refusals, which is what keeps a false positive costing one round trip instead of wedging the run. `pnpm josh time`'s `Investigation reads:` block prints those four figures for one run, from this guard's own predicates.
...
**The pre-gate cut row** ([#1864](https://github.com/joshuafolkken/kit/issues/1864)): the trigger is a `Bash` call that runs `pnpm josh gate` — in either spelling — from a **lane** working tree that has not yet taken its cut, and the refusal hands over `pnpm josh run:cut <N>` together with what each of its six verdicts obliges. It is the one row whose trigger consults the world beside the command, because neither half of "a lane that has not cut" is readable from the call: the lane is the working directory's own name under the lane root, and the cut is the record `run:cut` leaves on disk. Both reads are synchronous and both run only after the command has matched, so a run that never types the gate pays nothing for them. **Two states are silence, and they are the common ones**: a checkout that is not a lane — where every interactive `fullrun` runs its gate — and a lane whose cut record is already carried, which is exactly the resumed process the cut produced. It exists because the step was carried as prose and fired **0 times in 6 lane children** ([#1850](https://github.com/joshuafolkken/kit/issues/1850)); four of the six issued the entry check `run:cut --resume <N>`, were answered `fresh`, and went on to the gate, so the trigger deliberately does not count `--resume`, `--end` or `--json` as taking the cut. **The denominator it is scored over asks two things** ([#1867](https://github.com/joshuafolkken/kit/issues/1867)): a cut relaunches a new session, which this table reads as a run of its own and which issues the byte-identical entry check while having no cut left to take, so the row asks both that the run reached for the cut command and that it claimed the working-tree hold — the step a `fresh` verdict sends a run on to and a `resume` verdict tells its counterpart to skip. Scored on the asking alone, ten perfectly obedient children read as ten kept out of twenty. It stays **once per run**, because five of the six verdicts legitimately leave the process at the gate and each needs the reissue to pass. `.claude/skills/workflow-commands/pre-gate-cut.md` is the single source of the procedure.

**Wired to `Bash` alone**, and for a reason of its own rather than the batching guard's — that one names the edit tools too since [#1762](https://github.com/joshuafolkken/kit/issues/1762). Every rule this enumeration carries names its binding moment as a shell call: filing an Issue, reading one, pushing, piping a check. An `Edit` payload would start a process with no row to match it against.
...
Every kit-distributed SvelteKit project used to land on the same two ports, so a developer working across several of them on one machine could not run two previews at once — the second project's tooling either collided with the first or drifted onto an unpredictable port. `PORT_SEED` is a personal, non-committed integer in `.env`, and the offset applied to both ports together is `seed × 10 + lane`, where `lane` is the lane's seat (`JOSH_LANE_SEAT`, `0` for the main work tree):

```bash
PORT_SEED=1   # dev 5183, preview 4183   (seat 0: 1 × 10 + 0)
```

Multiplying the seed is what makes distinct seeds occupy disjoint bands — seed 1 is `10..19`, seed 2 is `20..29` — so a project can never share a band with a differently-seeded one, and the nine seats `1..9` inside a band are what that project's lanes take ([`josh lane:open`](#josh-laneopen)). Unset means seed `0` — today's numbers exactly — so CI and un-migrated projects are unaffected without doing anything. A blank `PORT_SEED=`, the shape `.env.example` ships and the natural way to turn a seed back off, means the same. One seed moves both ports, so a project can never end up with a dev port from one project and a preview port from another. An invalid seed (a non-integer, a negative, or one above `99` — the ceiling that keeps a whole band below the `1000` between the two port bases) is a hard error rather than a silent fall back to the default: a gate that quietly reverts to the shared port is the collision this exists to remove.

**The formula changed in [#1494](https://github.com/joshuafolkken/kit/issues/1494), and a set seed now resolves to different ports.** It was `5173 + seed` / `4173 + seed`; at the main work tree it is now `5173 + seed × 10` / `4173 + seed × 10`. A project on `PORT_SEED=1` moved from `5174` / `4174` to `5183` / `4183` — the `.env` value need not change, but a bookmarked `localhost` port does. `PORT_SEED` is a documented, distributed variable, so this reaches every consumer that set one; `pnpm josh doctor` lists each repository's seed and resolved ports so a clash is visible rather than waiting for an E2E run to fail.
...
**A command rather than an instruction, for the reason `josh review:brief --level-only` is one.** The answer it gives used to be a paragraph telling the run to compare `git diff main...HEAD` against three arrays in `scripts/init/init-logic.ts` by eye. Measured over one epic's children on 2026-09-08, three pull requests changed a distributed file and **two merged without the comparison being made at all**.
...
The reason names the list beside each path, because that is the half a reader cannot derive. The changed paths come from the same helper `josh review:brief` reads, so both answer from one definition of what changed.
...
**It also takes the `in-progress` label back off the issue, on a merged run only** ([#1794](https://github.com/joshuafolkken/kit/issues/1794)). Every workflow entry point writes that label before it implements, and nothing took it off on the ordinary ending — merged, closed, and still marked as running. The removal reads the issue's current labels and sends back the spelling GitHub stored, so a repository whose label is `In-Progress` is not missed ([#1132](https://github.com/joshuafolkken/kit/issues/1132)), and an issue that never carried it is never written to — the labels read is one call either way. Like the epic auto-close it is guarded: a failure is reported by name with `gh api -X DELETE repos/{owner}/{repo}/issues/<N>/labels/in-progress` beside it, and the run does not fail. A `--no-merge` run keeps the label, because the issue is still open and a run may still be holding it.

**It flushes the observation ledger after a merged run, so an appended line reaches the default branch without a person remembering** ([#1810](https://github.com/joshuafolkken/kit/issues/1810)). `docs/observations.md` is excluded from ordinary staging and [`josh observations:flush`](#josh-observationsflush) is its only commit path, but until now nothing called that path — an append stayed in the working tree until someone ran the command by hand, the same "a run that had to remember is the run that forgets" defect the staging exclusion was built to avoid, left standing on the commit side. After the merge tail and **before it releases the working-tree hold** (the flush switches branches, so the tree must still be held), `followup` reads the tree and, **only when the ledger holds a pending append**, returns the checkout to the default branch and flushes. A run that appended nothing pays nothing — no extra wait, no extra pull request — and a lane/worktree child never appends, so its checkout is always clean and the step is an immediate no-op there. Like the epic auto-close it is guarded: a flush that fails is reported and does not take the merge, the epic close or the hold release down with it. **A side effect on a run that did flush: `followup` ends on the default branch** — the workflow's own `pnpm josh ms` is idempotent, so nothing downstream changes.

**It ends the run's progress watcher on that same merged run** ([#1821](https://github.com/joshuafolkken/kit/issues/1821)). A [`josh run:progress`](#josh-runprogress) watcher outlives the turn that started it and, left alone, waits out its whole bound — up to an hour — after the run it was watching has already merged. On the same seam that releases the working-tree hold, `followup` removes the watcher's liveness record, keyed like the hold on the work tree's own git directory; the watcher reads it gone on its next tick and stops at once. Like the hold release it is guarded and needs no recovery command: the record is presence-only and the watcher's own bound ends it regardless, so a record left behind costs at most one idle watcher and never the merge. Nothing runs on a `--no-merge` run.
...
**A commit the pre-commit hook rejects is rolled back by the side that created the branch** ([#1785](https://github.com/joshuafolkken/kit/issues/1785)). It used to exit leaving the branch behind with no commit on it — and the refusal the next run printed named two exits, finish that branch or delete it, that the distributed `.claude/settings.json` denies an agent outright, so orphan flush branches accumulated in the checkout. The command now returns to the default branch and removes the branch it created before exiting; the appended lines are staged and uncommitted throughout, so nothing is lost and the same command runs again the moment the rejection is fixed.

**Where a flush branch really is still in the checkout, the refusal distinguishes the two cases rather than warning about both.** A branch holding a commit does hold the only copy of those observations, so it is landed first — `git push -u origin <branch>`, then its pull request. A branch holding none strands nothing, because the appended lines never left the working tree: there the exit is `pnpm josh ms` followed by another flush, which cuts a fresh branch and loses no line. A commit count that could not be read takes the first arm, since "nothing is stranded" is the answer that loses lines when it is wrong.
...
**`pnpm josh followup` runs this command itself after a merged run, so it is rarely typed by hand** ([#1810](https://github.com/joshuafolkken/kit/issues/1810)). The flush being the ledger's only commit path meant that until it was wired into `followup`, nothing called it and appended lines stranded in the working tree. `followup` short-circuits when the ledger is clean, so the automatic flush costs an ordinary run nothing; a hand run stays the route for a checkout that is not completing a `followup` — see the [`josh followup`](#josh-followup) entry above.
...
The `seed × 10 + lane` formula keeps distinct seeds in disjoint bands, but two repositories on the same seed still collide — and every repository that never set one sits together on seed 0. That clash otherwise surfaces only as an E2E run failing to bind a port in some other checkout, so `doctor` names it here. **It never rewrites a `.env`**: the seed is a personal, per-machine setting, so the report points at the clash and a person resolves it. A repository whose `.env` holds a malformed seed is reported as `PORT_SEED could not be read` rather than guessed at.
...
pnpm josh epic --add 893 894 --order-before 891   # put the row first; declare no dependency
pnpm josh epic --add 893 892 --order-after 891    # reorder two children, writing nothing else
```
...
- **`--order-before <M>` / `--order-after <M>` move the row and write nothing else** ([#1738](https://github.com/joshuafolkken/kit/issues/1738)). The task-list row goes directly before or after `#M`'s, the `## Dependencies` declaration is left byte-identical, and not one `blocked-by` relation is recorded or dropped. They exist because the two halves of a position were welded together: `epic:next` offers runnable children **in task-list order** ([#1583](https://github.com/joshuafolkken/kit/issues/1583)), so the only way to put one first was `--before`, which records a dependency — and a dependency stalls every child behind whichever one gets stuck. "No dependency, but run this one first" had no spelling at all, which is why [`wip-cap.md`](https://github.com/joshuafolkken/kit/blob/main/prompts/collaboration-workflow/wip-cap.md) forbids `--before` / `--after` for an interrupt without naming any alternative.
  - **They take the same three refusals a position takes**, and refuse without writing: `--order-*` given beside `--before` / `--after` (or twice, or with a target that is not an issue number), a target the epic does not track, and a target that is one of the issues being placed. **The hub ambiguity is not among them** — that refusal is about where a _dependency_ would attach, and an order-only move attaches none, so `--order-before <hub>` is answered rather than refused.
  - **They work with nothing to add.** `--add 893 892 --order-after 891` on two children the epic already tracks is a pure reorder — the case `--add` used to answer with `nothing to add`.
  - **The declared-but-unrecorded repair is withheld too.** An ordinary insertion records the relations a body declares that GitHub lacks; under `--order-*` nothing is recorded at all, since the caller asked for no `blocked-by` and a repair would put one back. The declaration is unchanged, so the next ordinary insertion still makes it.
  - **The report is one line**, because nothing else will say where the row went: `📋 Placed #894 before #891 in epic #893 — order only, no dependency written.`
  - **A row moved onto the wrong side of an order the declaration already states gets a second line.** `epic:next` filters by `blocked-by` **before** it applies task-list order, so moving `#892` in front of `#891` while `#890 -> #891 -> #892` still stands cannot make `#892` be offered any earlier — and the placement line, read alone, says it did. `⚠️ #892 is still held by a declared order, so `epic:next`will not offer it any earlier until that order is changed —`--remove` deletes one.` **It warns rather than refuses**: reordering inside a partly-ordered epic is legitimate, since the row still decides where the child sits among the runnable ones once its blocker closes. The test is transitive, because a chain is written in execution order.
  - **`--before` / `--after` are unchanged.** An order that really is a dependency is still written with them.
...
**A blocker no named epic tracks is weighed, not ignored** ([#1943](https://github.com/joshuafolkken/kit/issues/1943)). Before this the relation was announced and dropped, so a child waiting on another epic's open issue was offered as runnable. The answer now depends on whether this invocation can finish the blocker:

| The outside blocker is                               | The child is                                                                                       |
| ---------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| Closed                                               | Resolved — a blocker in another repository still waits for its release, as a tracked one does      |
| Open, and a child of **any** epic named in this call | Waiting on time — unless that blocker is itself waiting on a person, which the child then inherits |
| Open, and in no named epic                           | Waiting on a person, with the blocker named on standard error                                      |
| Of a state the relation response did not carry       | Waiting on time — an unknown is never read as finished                                             |

So `pnpm josh epic:next 1936 1931` lets a child of one wait on a child of the other, while naming a single epic sends a child blocked from outside it to a person. The state comes from the `blocked-by` response itself, so no extra request is made.
...
**A blocker is weighed against the whole backlog, not against the one epic that tracks the child** ([#1943](https://github.com/joshuafolkken/kit/issues/1943)). The set this run may run is every opted-in epic's children — including a child carrying no `auto-ok` of its own, because its epic's label is the approval — plus the standalone rows. An open blocker in that set makes the child wait; an open blocker outside it — or one in it that is itself waiting on a person — sends the child to a person, for epic children and standalone rows alike; the rest of the table under [`josh epic:next`](#josh-epicnext) applies unchanged. **A cycle whose links cross from one epic into another is reported as a `cycle` anomaly**, since no single epic can see it, and the answer is `error` exactly as for a cycle inside one epic; a loop among standalone rows alone still leaves them waiting, as before.
...
| `watch`     | Sleep the polling interval and ask both commands again. **With something of this run's own in flight the sleep is the progress watcher's exit instead**, and the interval is only a floor on the re-ask; with nothing in flight the watcher declines without exiting, so the parent keeps the interval itself (`epicrun.md` → "The wake exists only while something is in flight") | 0         |
...
pnpm josh review:brief --level-only          # the level alone, for a pre-commit review outside a workflow
pnpm josh review:brief --level-only --staged # the staged diff
pnpm josh review:brief --level-only --json   # the level and the reason, machine-readable
```

Pass the whole output to `/code-review`. The level is on the first line, so `$(pnpm josh review:brief)` still starts with it. **`--level-only` prints that level and nothing else** — the entry point for a pre-commit self-review run outside any workflow ([#1927](https://github.com/joshuafolkken/kit/issues/1927)). It bypasses the scoped-green refusal below, since a level classification reads only the changed paths and needs no gate; the classification it uses is "The review level" below.
...
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The level             | The changed-path classification in "The review level" below, decided once and not again                                                                                                                          |
...
`pnpm josh review:brief --level-only` prints the `/code-review` level this change is reviewed at ([#966](https://github.com/joshuafolkken/kit/issues/966)), and nothing else — the level to stdout and the reason to stderr, so `$(pnpm josh review:brief --level-only)` reads the level and a person still sees why.
...
**A prompt fix and a test fix both answer `required`, deliberately.** The condition the issue arrived with exempted anything that is not a runtime code path; that is rejected on the measurement recorded under `josh review:brief --level-only` — two documentation-only diffs, ten real defects found in each by a `medium` review, none of them covered by a test. A test file is the verification that guards a runtime path, and an assertion a fix weakened still passes. The full reasoning, how a skip is recorded on the Issue, and when the condition is withdrawn are in `prompts/review.md` → "When round 2 is skipped entirely, and when it is not".
...
| `followup-filing` | the parent reads the filed Issue from GitHub with `pnpm josh issue:state <new>` rather than from the unit's summary, so a follow-up reported filed but not created is still absent — the same catch as `epic-child`, and the finding text came from the parent so the unit's work is mechanical |
...
**The mechanism is not the unit.** How a thing is delegated is separate from what is delegated — one step of a run, or one whole child of a batch ([#984](https://github.com/joshuafolkken/kit/issues/984)). Both are rows of the one enumeration above rather than two mechanisms, which is why `epic-child` is answered by this same command. **One row covers both batch entry points**: an epic's child under `epicrun` and one issue of a `queue` are the same unit — same brief, same summary, same `pnpm josh issue:state` verifier — so the queue was wired to this row rather than given one of its own ([#1149](https://github.com/joshuafolkken/kit/issues/1149)). **`followup-filing` is a third such unit** ([#1892](https://github.com/joshuafolkken/kit/issues/1892)): the late-run filing chain (`issue:scout` → file → `epic:bundle` → `epic --add`) is a dependency chain `batch:guard` correctly keeps one call per step, so what makes it expensive is the ~340k context it runs in at a run's tail, not the step count. A fresh unit runs the same chain at a small context; the parent composed the finding text either way, so the unit's work is mechanical and the same `pnpm josh issue:state` read confirms it. Deferring it instead was structurally blocked — the finding is only known after the review, and `epic:bundle` must run before the current Issue closes.
...
pnpm josh run:carry --merged 1 --owner "$PPID"                    # a child merged
pnpm josh run:carry --filed 1 --owner "$PPID"                     # an issue was filed
pnpm josh run:carry --done 1762 --owner "$PPID"                   # a queue's issue is finished
pnpm josh run:carry --cut --owner "$PPID"                         # a cut is coming: hand the record off
...
**The record names the process spending the budget, and a second parent is refused against it.** `--owner <pid>` is that declaration: `--begin` records the pid together with the start time that tells a reissued pid apart, and any later `--begin` or `--resume` while that process is still running answers `busy` and exits 1 — **whatever either command line says** — with one exception, and it is the whole of [#1935](https://github.com/joshuafolkken/kit/issues/1935): a record a `--cut` handed off answers `resumed` even while its owner is still alive, so a `backlogrun` cut from an interactive session that never ends, or a lane child woken by a background notification, is still carried to exactly one successor rather than stalling every resume with `busy`. It is safe because a hand-off needs the `--cut` a crash never reaches, and uniqueness holds because the adoption spends the hand-off and moves the ownership, so the _second_ would-be successor reads a record no longer handed off and is answered `busy`. Pass the parent loop's own long-lived process, which under a `backlogrun` is `--owner "$PPID"`. Without it the record simply declares no owner, and every standing record then reads as not provably live, which refuses rather than resumes. It is not `josh run:hold`'s pid, which is recorded for the reader and deliberately never read back: the process that claims a work tree is the short-lived `josh run:hold` itself, while the owner here is the session that outlives every command it issues.
...
**A count is refused unless it names the record's live owner** ([#1935](https://github.com/joshuafolkken/kit/issues/1935)). `--merged`, `--filed`, `--cut` and `--done` take `--owner "$PPID"`, and a count whose owner is not the record's — because a `--cut` handed the record off and it awaits its successor, or because a successor has already taken it over — answers `busy` and exits 1 rather than advancing a budget that is no longer this session's. This is the single writer of [#1722](https://github.com/joshuafolkken/kit/issues/1722) kept across a cut: the cutting session hands off and stops counting, and the successor adopts, becomes the owner, and counts.
...
pnpm josh run:wake --list           # which supervisor is running, what it has done, and its latest progress
...
**What it launches is a constant, not configuration.** It runs `claude -p --verbose --output-format stream-json --model <model> --effort <effort>` with the recorded invocation as the prompt — the streaming flags making the session write a JSON event per turn so its detached log grows while it works ([#1948](https://github.com/joshuafolkken/kit/issues/1948)) — the model and effort defaulted and overridable exactly as [`josh lane:dispatch`](#josh-lanedispatch) documents ([#1932](https://github.com/joshuafolkken/kit/issues/1932)) — resolved through `PATH` exactly as the `josh eval` harness resolves the same binary. It was an environment variable first, and that put the choice of _which binary runs unattended, overnight, with the person's own credentials_ in reach of anything that can set an environment; shape-checking the name does not address that, and removing the choice does. Supporting a second agent is its own decision with its own security thinking, and belongs in an Issue of its own.
...
**The file exists because the path was resolved, not because something launched into it** ([#1759](https://github.com/joshuafolkken/kit/issues/1759)). Until then it was created only by the process that launched, at the moment it launched — so the two verbs that name the path without launching anything, `--list` and the already-running branch of `--start`, named a file that need not be there, and a supervisor started before this log existed left the path named for a whole run with nothing ever at it. Every verb now creates it when it resolves the path, and a log nothing has launched into is **empty rather than absent**: an empty file says this supervisor started nothing, where a missing one said nothing at all. Each launch appends a header naming the time and the launching process, so what is in the file is exactly what was launched into it.
...
**And `--list` relays the woken parent's latest progress** ([#1910](https://github.com/joshuafolkken/kit/issues/1910)). After a cut the parent is a headless `claude -p backlogrun`, so its twenty-minute heartbeat reaches only its own transcript; the watcher persists each line into the report clock [`run:progress`](#josh-runprogress) keeps, and `--list` prints the last one verbatim beside the supervisor's state — the person's one window onto the run once they are no longer the parent session. It is read, never pushed: no heartbeat goes to Telegram, so `confirmation` and `completion` stay the only messages that interrupt someone.
...
pnpm josh run:cut --resume 1839   # a fresh process's entry check: fresh | resume | stale | busy | handed-off
...
**A run underway is reported even before its first child carries `in-progress`** ([#1900](https://github.com/joshuafolkken/kit/issues/1900)). "A run has started" is read from a mechanical record — a registered lane, a held work tree ([`run:hold`](#josh-runhold--josh-runrelease)), or a carried budget (`run:carry`) — rather than from the label, so the window between a run starting and its child's first label no longer goes dark: a `backlogrun` measured on 2026-09-13 dispatched a child that spent fourteen minutes reading before it labelled itself, and the watcher reported nothing that whole time. In that window the children field reads `no in-progress child yet`, an observed fact rather than a guess, with the lanes field beside it carrying whatever evidence exists. Only a checkout with no run recorded at all — no hold, no carried budget, no lane, and no `in-progress` child — reports nothing, which is what keeps an ordinary conversational session outside the heartbeat.
...
| `#<N> <labels> PR:<state>`                            | One per child in flight, from the same `in-progress` listing `epicrun` sizes its lanes from, with `none` / `open` / `merged` / `closed` for its pull request; `no in-progress child yet` in its place when a run has started but none carries the label yet ([#1900](https://github.com/joshuafolkken/kit/issues/1900))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `--hours`                        | How long the watcher lives before it exits on its own. Default 1 — three of the default twenty-minute intervals, so the bound never truncates a report, and a watcher left waiting on an already-merged run is gone within the hour. Deliberately **not** `run:hold`'s eight-hour expiry: that holds an uncommitted tree across a person's latency, while a watcher holds only a heartbeat the caller restarts, so only one has anything to lose by being short |

**The watcher writes its own liveness record, and `josh followup` removes it at the merge** ([#1821](https://github.com/joshuafolkken/kit/issues/1821)). `--hours` is the backstop; the ordinary end is the merge. The watcher writes a presence-only record when it begins and reads it at the top of every tick — the `run-wake-loop.ts` `run_loop` shape ([#1727](https://github.com/joshuafolkken/kit/issues/1727)), with the read/write single-sourced on `stamp_file` — and stops the moment it is gone, so a watcher left waiting on an already-merged run ends within a tick rather than waiting out the hour. The record is kept apart from the report clock `--mark` writes, on the same key (the work tree's own git directory), so removing it never disturbs the clock the early-heartbeat guard reads. A watcher ended this way is a clean stop and prints no bound-expired notice.

**The report clock also carries the last line, so a headless parent's progress survives a session cut** ([#1910](https://github.com/joshuafolkken/kit/issues/1910)). Each heartbeat `--wait` or `--once` prints is written into the report record beside `reported_at`, and [`run:wake --list`](#josh-runwake) reads it back verbatim — after a `backlogrun` cut the parent is a headless `claude -p backlogrun` whose heartbeat reaches only its own transcript, and this record is the one place that line survives for a person to read. `--mark` moves the clock without a line of its own and **keeps** the one already there rather than blanking it, so `--list` still shows the most recent heartbeat between a run's real reports. It stays a pull, not a Telegram push, so the reason above holds unchanged.

**A repository with no run underway is told apart from one whose listing could not be read.** Under `--once`, a checkout with no run recorded and no child carrying `in-progress` prints nothing and exits 0 — that is an answer — while a listing that could not be read prints nothing and exits 1, the same fail-closed reading `run:hold` and `run:liveness` give an unreadable record. **The watcher has nowhere to exit to, so it says the reason on standard error instead**, once per streak rather than once per attempt: a condition lasting an afternoon costs one line, and a watcher that swallowed it would look exactly like an idle repository.

**A tick that reads nothing does not end the watcher, and does not spin either.** Every reading it makes spawns a process or touches the temp directory, so one `git worktree list` that cannot fork under load — the load this command exists to report — would otherwise take the reporting down for the rest of the run, with nothing waiting on it to notice. The failure is reported on standard error and the loop continues. A declined tick leaves the report clock alone, so the first child to appear is still reported at once, but it does set a **two-minute read cooldown**: without one, an idle or failing repository would be re-read every 30 seconds for the watcher's whole life, and across a batch's stacked watchers that is enough calls to start causing the unreadable listings it is handling.
...
**Port separation is the reason this command exists, not a detail of it.** `ports/index.js` resolves the project root as the nearest ancestor holding a `package.json` — which, inside a linked work tree, is the lane's own directory. So a lane with no `.env` runs on seed 0 and a lane handed a verbatim copy of the root's runs on the root's exact offset; either way every lane lands on one pair of ports, and a busy port **fails on the spot rather than retrying on another**, which is the behavior `playwright.config.ts` documents and which nothing here weakens. `lane:open` therefore copies the root `.env` — `TELEGRAM_*`, `JOSH_SESSION_LANG`, the project's `PORT_SEED` and everything else carried across verbatim — and adds a `JOSH_LANE_SEAT` line, so the lane keeps its project's seed and takes a seat within that project's band.

**The main work tree is always seat 0.** Lanes take seats `1..9`, so a checkout that never opens one keeps exactly the ports it has today, and CI — one work tree, no `.env` — stays on 5173 / 4173.

**The offset is `seed × 10 + seat`, so a whole band stays below 1000 and distinct seeds never overlap** ([#1494](https://github.com/joshuafolkken/kit/issues/1494)). The multiplication lives in `ports/index.js` alone ([`josh port`](#josh-port)); a lane's `.env` records only its seat in `JOSH_LANE_SEAT`, so the file plainly shows which number is the project's seed and which is the seat. The seed ceiling `99` and a band with every seat taken both fail rather than wrap — wrapping is that same collision under a friendlier name.

**The free seat is read from the live lanes' own `.env`, never from a counter.** A monotonic counter, or a number derived from how many lanes have ever been opened, hands a closed-and-reopened lane the seat a running lane is still using. `lane:open` reads `git worktree list --porcelain`, reads the `JOSH_LANE_SEAT` of each lane it finds and takes the lowest unused seat in `1..9` — so **the state lives only in each lane's `.env`**, and `git worktree remove` erases the record of a seat along with the tree that held it. There is no ledger file, and nothing that can go stale. **The chosen seat is claimed atomically** — by exclusively creating a lock directory before the lane's `.env` is written — so two `lane:open` runs that both saw the same seat free cannot both take it: the loser steps to the next free seat, or fails when none is left ([#1494](https://github.com/joshuafolkken/kit/issues/1494)).
...
**It seeds the lane's verification caches from the main checkout, so the lane's first `josh gate` is warm** ([#1849](https://github.com/joshuafolkken/kit/issues/1849)). A linked work tree is created with `git worktree add`, which writes none of the git-ignored caches the gate reads — `.eslintcache`, `.tsbuildinfo`, `.cspellcache` — so a fresh lane's first gate runs fully cold (353.9 s against 27.5 s warm, [#1839](https://github.com/joshuafolkken/kit/issues/1839)). The three are copied in from the main checkout after the `.env` is written and before the install. **A copy hits rather than missing every entry, because all three are content-addressed**: eslint and cspell run `--cache-strategy content` and tsc records its options inside the build-info file, so an entry is validated by the hash of the file it describes rather than by that file's mtime — which `git worktree add` necessarily rewrites. It is the same property that lets CI restore `.eslintcache` across runs on different machines. **Each lane takes its own copy, never a shared file**, so two parallel lanes cannot rewrite one cache whole and discard each other's entries. **Warming is best-effort**: a cache the main checkout has not produced is skipped, and a copy that fails is skipped rather than thrown — so unlike the install, a lane whose warming found nothing still opens and is perfectly usable, cold first gate and all.
...
**For a lane started by [`josh lane:dispatch`](#josh-lanedispatch) the child streams its output, so a growing log is itself a sign of life** ([#1749](https://github.com/joshuafolkken/kit/issues/1749), [#1948](https://github.com/joshuafolkken/kit/issues/1948)). The child runs `claude … --output-format stream-json`, so it writes a JSON event per turn and tool call while it works and the log grows; a log frozen past the silent window is then a genuine stall. The process trace decides that case: the child's command line is `claude … fullrun #<N>` and carries no path, so `pgrep -laf "fullrun #<N>$"` — never a `pgrep` on the lane's directory, which the child's argv omits — is what finds it. Run it and pass what it found; `--process alive` above is that answer in the ordinary case, never a constant to pass because the child was dispatched.
...
**The model and effort are explicit, and defaulted in one place** ([#1932](https://github.com/joshuafolkken/kit/issues/1932)). The child is started as `claude -p --model <model> --effort <effort> fullrun #<N>`, with `model` defaulting to `opus` — the latest-version alias, so a lane matches the parent session rather than whatever `~/.claude/settings.json` happened to hold — and `effort` to `medium`. `JOSH_LANE_MODEL` and `JOSH_LANE_EFFORT` in `.env` override each: the model passes the same control-character and length gate every argument does, while the effort is an allowlist — `low`, `medium`, `high`, `xhigh`, `max` — and a value outside it refuses the launch rather than starting a session that would fail minutes in. A blank override is treated as unset. The chosen model and effort are written into the launch log's header, so runs can be compared afterwards, and [`josh run:wake`](#josh-runwake)'s resumed session takes the same defaults through the same launcher.
...
**The output path is recorded by the same call that starts the child**, so "this lane records no path" stops being a state a run can reach. **The dispatch owns that path rather than reusing whatever was recorded**: [`josh lane:output`](#josh-laneoutput) accepts any path under the home or temp directory, and what the older flow recorded there was the unit's own _transcript_ — a file to read — so appending a launch header and the child's raw output into one would corrupt exactly the file `run:liveness` then parses. The path is `<temp>/josh-lane-dispatch-<N>-<digest of the lane's directory>.log`, in the temp directory rather than in the lane so that it survives [`josh lane:close`](#josh-laneopen--josh-laneclose--josh-lanelist--josh-laneprune): the question a failed child raises is asked after its lane is gone. It is the same path for every dispatch of one lane, so a re-dispatch appends a second header to the file it already owns — and **the digest is why the issue number alone does not name it**: issue numbers are per repository, and an epic spanning two would otherwise have `kit#12` and `app-kit#12` writing into one file, each child's writes keeping the other's log growing so that a poll read a dead child as alive.
...
pnpm josh cost --cap 300000     # what share of the run's cost fell at or under this per-request cap
```

**The no-argument default is the run tree** ([#1937](https://github.com/joshuafolkken/kit/issues/1937)). A `backlogrun` or `epicrun` is one parent session, its dispatched lane children, their subagents at any depth and the wake/resume sessions that continued it — and the old default, the newest single transcript, answered for whichever of those finished last, one lane child of a dozen. `--run` gathers the whole tree and breaks its cost down **by role** — parent, lane, subagent, wake — each with its session count, requests, dollars (with the read/write/output split), preamble tokens, active minutes, and share of the run. Sessions are listed cost-heaviest first, each tagged with its role, issue, depth and the session that spawned it. Merges are not derivable from transcripts, so that count reads `not measured` rather than zero, and a transcript the run did not claim is reported as an unattributed count rather than folded in. The narrower scopes below stay for looking at one session or issue; [`josh time --run`](#josh-time) reads the same tree with wall clock led instead of dollars.
...
**`--over <tokens-per-request>` answers a hand-off question instead of printing a table.** It prints `over` or `under` on stdout and the measured figure on stderr, comparing the session's billed input divided by its request count against the limit. Per request rather than in total, because the total only says the session was long — the ratio says what the _next_ turn will cost, which is what a hand-off decision turns on. `epicrun` uses it after a merged child ([#968](https://github.com/joshuafolkken/kit/issues/968)); measured across one run of six children in one context, the figure went from 222k during the first child to 645k during the sixth. [#1212](https://github.com/joshuafolkken/kit/issues/1212) narrowed it to a child that ran in the parent's own context, on the ground that delegated children add only 4,000–5,000 each; **[#1567](https://github.com/joshuafolkken/kit/issues/1567) removed that narrowing**, having measured a parent that delegated every child and still billed $0.241 a request against its children's $0.080, on 361,833 cache-read tokens a request across 732 requests. The child summaries were the smallest of the five things accumulating in that parent — the executing side's own prose was 27.9%, tool results 25.2%, thinking 21.6% — so a gate conditioned on them alone never fired. **It is now asked after every merged child, and `over` stops the session and asks the person to cut it** — the lanes still in flight are handed over rather than waited on, because each one records where its unit writes ([`josh lane:output`](#josh-laneoutput), [#1713](https://github.com/joshuafolkken/kit/issues/1713)) and the next session polls them from [`josh lane:list`](#josh-laneopen--josh-laneclose--josh-lanelist--josh-laneprune). The one reading that does not stop is a lane nobody could poll — `unreadable`, or `open` with no recorded path — in which case the run records the reading and asks again at the next merge. The resume command is printed at the stop. **[#1605](https://github.com/joshuafolkken/kit/issues/1605) moved the threshold to 150,000 and widened where it is asked**: `fullrun` and `halfrun` had never asked it at all, and 400,000 was drawn against one `epicrun`'s band rather than theirs — measured over 223 recorded sessions with the definition this flag uses, only 4 of them (2%) ever exceeded 400,000, while the first request of a session, carrying the resident preamble alone, has a median of 54,974. 150,000 is the lowest candidate at which none of the measured sessions is cut before its first run has finished, and it leaves the median session five runs before the cut against 400,000's six. It is now also asked at the **entry** of a `fullrun` or a `halfrun`, beside `josh run:hold`, where `over` stops the run before it has filed, branched or edited anything. **[#1775](https://github.com/joshuafolkken/kit/issues/1775) then raised what the documents pass to 300,000 as a time-boxed experiment** — not a settled number: it sits just under the p95 of 336,569, so the share of sessions never cut rises from 33% to 81%, which is the direction the 400,000 finding above argues against, and that issue carries the retreat procedure and what is measured before deciding whether to keep it. The threshold itself lives in [`epicrun.md`](https://github.com/joshuafolkken/kit/blob/main/.claude/skills/workflow-commands/epicrun.md) → "The hand-off", which is the single source of when it is asked and what is done with the answer.

**The report carries a cost curve across request position** ([#1838](https://github.com/joshuafolkken/kit/issues/1838)). `josh cost` read a run as a flat set of requests, so the structural fact that the late requests are the large, expensive ones — the number a hand-off turns on — never appeared. The whole-run report now splits its requests into four positional quartiles (`Q1`..`Q4`) and prints, for each, the average context (a request's billed input) and its cost, alongside the first-request and last-request context so the growth is visible at a glance. The quartiles are positional, not a percentile of a sorted value set: sorting would discard the order, which is the axis. An empty quartile — a run of fewer than four requests — and a quartile holding a model the price table does not know both report `not measured` rather than a zero.

**The whole-run report attributes the carry of each entry-read instruction document, now with its re-reads** ([#1871](https://github.com/joshuafolkken/kit/issues/1871), [#1913](https://github.com/joshuafolkken/kit/issues/1913)). The `Entry-read documents` block sizes each `.claude/` / `prompts/` / root agent document a `Read` brought into the conversation partway through and prices the tokens it then carried on every following request — the per-document carry a point-of-use decision trades away. **[#1913](https://github.com/joshuafolkken/kit/issues/1913) added the re-reads that the single carry hid**: `read_count` is how many times the scope read the document, `reads[]` places each read on the timeline with the context size when it happened (`context_tokens`), and `duplicate_cost_usd` is what the reads past the first cost. The four carry columns are unchanged — the primary row is still the earliest read's carry, so a resumed session copying a parent's read line never double-counts it — and a document read more than once appends `read Nx (dup $…)` to its row. **A whole-document re-read at a late, large context is the waste this surfaces**: a 32 KB `followup.md` read twice near a run's tail is `read_count: 2` with a non-zero `duplicate_cost_usd`, where before it was one carry and the second read was invisible. A read counts as a re-read of the scope only if its own request is in the scope, which is what keeps a fork's copied read line out of the count. `--json` carries `read_count`, `duplicate_cost_usd` and `reads[]` on each row.

**`--cap <tokens-per-request>` answers a counterfactual `--over` could not.** Given a per-request context cap, it prints the share of the run's priced cost that was incurred by requests at or under it — `81.4%` at 300K says that handing off before any request exceeded 300K would have kept 81.4% of the spend. It is the observed run's spend at or under the cap, not a projection of a run replanned to hand off there; the deferred work would still have to happen in a fresh session. It may narrow to `--issue` and it carries the whole simulation under `--json`, but it refuses `--all` (which report's ratio?) and `--over` (a second verdict on one run). When nothing in scope could be priced it answers `not measured`, never a ratio of 0.

**`[1m]` model ids carry no long-context surcharge, and the marker is stripped deliberately.** The published price list (platform.claude.com "Long context pricing", read 2026-09) states that Claude 4.6 and later models include the full 1M-token context window at standard pricing — a 900K-token request is billed at the same per-token rate as a 9K one — and every id the price table prices is 4.6-or-later. So `resolve_price` reduces `claude-opus-5[1m]` to its base rate rather than a premium band, and the justification is recorded where the stripping happens. (Fast mode is a separate premium, signalled by `speed: "fast"` rather than by the context marker.)

**The dollars are split by token type, and the turns by output size** ([#1912](https://github.com/joshuafolkken/kit/issues/1912)). `cost_composition` breaks `cost_usd` into the five priced kinds — uncached input, the two cache-write TTLs, cache read, and output — as dollars that sum back to the total, so "where did the money go" is a reading rather than a re-derivation from the token table above. `output_turns` reports the per-turn output distribution (median, p90, max) and how many turns — and what share of the output — exceeded the large-turn threshold (5,000 tokens, a named constant): a hand measurement of `fullrun #1876` found roughly half a session's output came from a handful of such turns, which the averages hid. Both are present on every non-empty scope and carried under `--json`; an empty scope carries neither, the same withheld-is-not-zero idiom the curve follows.

**A run is broken down by the main-line session it was spent in** ([#1912](https://github.com/joshuafolkken/kit/issues/1912)). `by_session` carries one row per session the `--issue` or `--all` scope spanned, oldest first — its request count, output tokens, `cost_usd`, `cost_usd_with_delegated` (the units that session spawned folded in), the per-type `composition`, its own `output_turns`, and, for a resumed session, the `preamble_tokens` it re-established before doing any work. A run that stopped and resumed reads as two rows, which is what let the hand measurement attribute 76% of `fullrun #1876`'s cost to its stopped first session — the run total alone could not. A single-session scope carries none, because it has no sub-sessions to break out.

**Each session row also carries a model, thinking-share and context-size line** ([#1969](https://github.com/joshuafolkken/kit/issues/1969)). Under the row is a continuation line naming the model the most requests ran on (with any others it also used) and, because the transcript records no reasoning effort, `effort not recorded` rather than an invented one; the thinking-token share of the session's output; and the per-request context-size distribution (median, p90, max of the billed input), built from the shared `time_distribution` rather than a second percentile helper. The thinking share reads `not measured` when the API sent no thinking count at all — kept apart from a genuine `0%` by a `thinking_measured` flag threaded through `cost-usage.ts`, since `?? 0` alone could not tell "the model did no thinking" from "the field was absent".
...
- **[#1856](https://github.com/joshuafolkken/kit/issues/1856) moved a fourth, `chain-rule.md`, to the point-of-use block** — it governs the `/code-review` → `followup` chain, which binds after the first edit, so a `fullrun` / `queue` / `epicrun` / `backlogrun` no longer carries its 7,396 tokens from the entry. Measured the same day, the `fullrun` entry fell a further step, from 75,619 scoped to 68,727.
- **[#1873](https://github.com/joshuafolkken/kit/issues/1873) moved a fifth, `background-commands.md`** — it was §2h's resident body, governing the background execution of the gate, the push and the merge tail, all of which bind only after the first edit, so it left `SKILL.md` for the point-of-use block while a pointer and the one batch-scale line stayed. `SKILL.md` fell from 37,147 to 35,136 tokens — read at every workflow entry, so the whole set falls with it — and the `fullrun` entry from 69,829 scoped to 67,818.
...
**The no-argument default is the run tree, not the last merged run** ([#1937](https://github.com/joshuafolkken/kit/issues/1937)). Under a batch the last merged run is one lane child, so the bare command now reports the whole run — parent, lane children, subagents at any depth and wake/resume sessions — as [`josh cost --run`](#josh-cost) describes, reading the same tree with wall clock led beside the dollars: each role carries its active minutes and share alongside its cost, requests and preamble. `--issue <number>` still reports one issue's run from invocation to merge, and `--run` reaches the tree the same way the bare invocation does. The scope is refused beside any of `--issue` / `--session` / `--epic` / `--last` / `--period`, which name different things.

**The bare invocation leads with the run's state, read from the `run:carry` / `run:wake` records rather than from a merged pull request** ([#1939](https://github.com/joshuafolkken/kit/issues/1939)). A run measured from a merged pull request can never see a stopped one — a run cut and never resumed has no merge to select — so the `backlogrun` of 2026-09-13 lost more than an hour and a half to a run cut and never resumed, and neither `josh time` nor `diag` could show it. When a carry record is still here — a clean finish removes it through `run:carry --end`, so its presence _is_ the run's incompleteness — the report opens with a `Run state (from run:carry / run:wake):` block naming the status, the number of cuts, the last cut, whether a successor took over, the idle time since the last activity, and whether the run ended. The status is a separate axis from `by_session`'s `merged` / `stopped` / `not_detected`, carrying the two that axis could never express — **`cut, but no successor took over`** and **`owner process alive but idle`**. Wake/resume sessions that spun up and did no work — no edit, commit or merge — are listed as **whiffs** with their count, total dollars and times, priced through the same reader `josh cost` uses. When no record is here the run has finished, or none ran, so nothing is prepended and the block never reads a missing record as "not stopped". The detection reuses `run-carry` / `run-wake`'s own reads rather than a second copy of them, and `josh cost --run` is handed no such block and is unchanged.
...
**A run is broken down by the main-line session it was spent in** ([#1912](https://github.com/joshuafolkken/kit/issues/1912)). `by_session` carries one row per session the run spanned, oldest first — its elapsed time, model and tool wait, round-trip count, how it ended (`merged`, or `stopped` naming the last failed command, or `not_detected`, decided mechanically from the spans), and, for a resumed session, the wall clock from its start to its first forward progress (an edit or a state-changing `josh` command). A run that stopped on a red gate and resumed reads as two rows — the first `stopped`, the second `merged` — the axis the pooled figures could never show. It is the transcript-derived companion to [`josh cost`](#josh-cost)'s own `by_session`, keyed by the same session id, so the two can be read side by side.

**Each session row also carries two signals joined from the cost axis' tokens** ([#1970](https://github.com/joshuafolkken/kit/issues/1970)): a **check loop** — how many single verification checks followed an edit, with the median billed input the run was carrying at those checks — and the **stall candidates** — model waits that ran long while returning little (at least ten minutes, under five output tokens a wait-second), each listed on its own line with its clock window, wait and output tokens. Both need the per-request tokens, so they appear only on the paths that read the cost corpus (`--issue` and the latest run) and are withheld on the epic, last-N and history paths rather than shown as zero. The run-wide `single_checks` figure stays beside them: it counts checks that _repeat an earlier call with the same arguments_, a different question from this per-session cadence.

**Each delegated unit now names its model and its purpose** ([#1912](https://github.com/joshuafolkken/kit/issues/1912)). `delegated_cost.units[]` carries `model`, read from the unit's context-construction request — a subagent may run a cheaper tier than its parent, which is what makes a launch's cost readable — and `purpose` (`review` / `investigation` / `unknown`), read from the unit's own transcript by the command and brief markers the workflow forks it with, and `unknown` where none matched rather than a guess.
...
Round trips:
  ...
  tool-less turns                55   issued no tool call
...
Investigation reads:
  edit targets                   17   89.5% · of the reads this run made · 12 in setup
  under the threshold             1   5.3% · of the reads this run made · 1 in setup
  refused                         0   0.0% · of the reads this run made · 0 in setup
  let through                     1   5.3% · of the reads this run made · 1 in setup
...
Guard refusals:
  pre-gate cut              2.0 min   2 refusal(s) · $0.42 · 2 re-issued same args
  investigation             1.1 min   1 refusal(s) · $0.08 · 0 re-issued same args
...
**The `Investigation reads:` block decomposes the largest of those contributors, which nothing could do before** ([#1764](https://github.com/joshuafolkken/kit/issues/1764)). `investigation` measured **624 turns, 35.9%** over nineteen runs — second only to implementation — and two quite different things sat inside that one figure: the reading [`SKILL.md` → §2b](https://github.com/joshuafolkken/kit/blob/main/.claude/skills/workflow-commands/SKILL.md) leaves in the main line on purpose, because an `Edit` cannot be issued against text nobody holds, and the reading the delegation threshold was meant to send to a unit. **The four rows are the guard's own answers, replayed rather than re-derived**: `scripts/delegation/investigation-reads.ts` is the mechanism, and `classified_reads` walks a recorded run through the same subject test, threshold, delegation reset and re-arm the live hook uses — a second walk that merely resembled the guard would answer about a rule nobody ships. **The last row is the only one that is a defect**, and it is a defect in the guard rather than in the run: a read that reached the threshold and was **not refused**. Two things put a read there and the row covers both — a refusal already standing that nothing had re-armed, and a spelling the guard never refuses at all, since `sed -n` prints and `sed -i` rewrites and `time-batch-guard.ts` → `is_read_only_call` answers conservatively for both. Measured over 296 recorded sessions the row was **36.9% of every main-line read** before the re-arm [`josh investigation:guard`](#josh-investigationguard) now documents, and **30.0%** after. **The unit is a read, not a turn** — one turn can open three files, and the question here is what each of them was — so this block and the one above it are read together and neither is derived from the other. **Each row also names how many of its reads fell in `setup`** — the reading done before implementation starts ([#1868](https://github.com/joshuafolkken/kit/issues/1868)), which is the phase a `fullrun` spends a median 36% of its body in and the one a reduction targets first, so "does the breakdown point to wasted reads" is read off the block rather than computed by hand. **A run whose transcript was not read prints four `not measured` rows**, because four zeroes would read as a run that opened nothing. `--json` carries the same figures under `investigation`, and the class tally split by phase under `investigation.by_phase`.

**The `Guard refusals:` block ranks which guard stopped the run and what re-issuing cost** ([#1913](https://github.com/joshuafolkken/kit/issues/1913)). The `Failure re-runs:` block counts every errored call and cannot say which guard spoke; a run whose two false-positive pre-gate-cut refusals ended a session read there as two of a hundred failed calls with nothing to separate them. This block reads the guard off the `⛔` opening the hook wrote back — carried onto the span at parse time as `refusal_guard`, the one place a refusal is identified — and groups by it. **Each row is one guard**: the wall clock re-issuing cost in the numeric column, then `refusal_count`, the `cost_usd` the composing model turn cost (priced through the same `time-region-costs.ts` `tally` the contributor block uses, so a refused turn's request lands in that guard's bucket), and `same_args_reissue_count` — a refusal the run answered with the identical call, the false-positive hint that told pre-gate-cut apart from a real catch. **The investigation guard is included, which needed its reason aligned**: its refusal opened with no `⛔`, so the marker every other guard carries was added to `investigation_reads.REASON` for it to be detected mechanically like the rest. **The block is absent for a run that walked through no refusals** — the ordinary case, on the reasoning `Bundling:` prints nothing it had nothing to say about — and only the run scopes read the cost corpus, so `is_cost_measured: false` reports the counts with the cost unmeasured rather than as a zero. `--json` carries the figures under `guard_refusals`.
...
**The `tool-less turns` row is the turns that issued no tool call at all** ([#1875](https://github.com/joshuafolkken/kit/issues/1875)), `turn_count − round_trip_count`. `Bundling:` cannot see them — a turn with nothing to batch never enters a sequence — and `batch:guard` cannot refuse them, because a turn that calls no tool fires no `PreToolUse`; so the only lever on a run of pure-utterance turns is this after-the-fact count, the same reading [`turn-batching.md`](https://github.com/joshuafolkken/kit/blob/main/prompts/collaboration-workflow/turn-batching.md) relies on for the batching itself. On the run above it was 55 of 156 turns; run #1864, the run #1875 measured, had 28.
...
**The three `Parent —` blocks time a `backlogrun` parent's own run, which the turn counts above cannot** ([#1940](https://github.com/joshuafolkken/kit/issues/1940)). The 2026-09-13 parent was measured by hand at 18 minutes before its first lane — a foreground `followup` that timed out at 600 s while no new lane started — and then editing a parked child itself at ~290k carried context; neither cost had a block. `Parent — first lane dispatch:` says how long the run took to start its first lane and lists each dispatch, read from the run's own `josh lane:dispatch` calls rather than a second copy of that command's decision to dispatch. `Parent — foreground waits (in run order):` lists the intervals the parent blocked in the foreground — a foreground `followup`, a CI wait, a `sleep` timeout — with the command it sat on and their total, excluding anything the run backgrounded. `Parent — own implementation:` sums the parent's implementation turns — the requests they billed, their cost, the median context they carried, and the issue the work was for — reusing the `implementation` contributor above so the two blocks name a turn the same way. Each block withholds itself rather than printing a zero: a run that dispatched no lane reads `no lane dispatched`, and a transcript that was not read reads `not measured`. **Identifying _which_ session is the parent is a later child's** (the run-tree scope): the block computes from the run's own spans and cost corpus until that scope lands.

**The `Single checks:` block is about the probing in front of that gate, not the gate itself** ([#1383](https://github.com/joshuafolkken/kit/issues/1383)). `CLAUDE.md` has said **one gate per run, not one per edit** since [#1246](https://github.com/joshuafolkken/kit/issues/1246), and the single checks — `josh lint:related`, `josh test:related`, `josh cspell:dot` and the type check — are where that rule sends an implementation loop instead. Nothing measured whether the loop then repeated _them_. **Two calls are the same call only if they named the same files**, so the signature is the subcommand plus its arguments, sorted and with shell redirections dropped — read while the tool input is still in hand, since a span keeps none. `answered nothing new` is the narrow figure the rule in `.claude/skills/workflow-commands/chain-rule.md` → "A single check answers once per tree" is about: a repeat that only a model turn sat between, whose answer was therefore known before it was asked. **It under-reports on purpose** — a repeat separated by a plain `Read` is not counted, because nothing here can prove that call changed no file — and a consumer project's type check is `josh-app check:ci`, which is not a `pnpm josh <cmd>` call at all and so is invisible. The zero above is the honest reading of that run rather than an empty block: all three of its repeats followed an edit.
...
Turns by contributor (median, then min – max):
  implementation                 24   22 – 49 · 9 run(s)
  investigation                  30   11 – 86 · 9 run(s)
  …
```

**The `Turns by contributor:` block is aggregated here and in `--period` too, so a turn-count trend is read by command rather than by hand** ([#1763](https://github.com/joshuafolkken/kit/issues/1763)). [#1715](https://github.com/joshuafolkken/kit/issues/1715) split one run's turns into eight contributors and every scope printed that block — but nothing aggregated it, so the only way to a trend was to parse `--json` yourself. The 2026-09-11 reading did exactly that over nineteen runs, and the table it produced **is not comparable with the next hand count**: the two took different denominators and neither said which — the defect [#1729](https://github.com/joshuafolkken/kit/issues/1729) named for depth shares, in the block the cost question actually turns on. **One module aggregates and renders it for both scopes**, so a last-N reading and a period reading of the same runs cannot come to answer the question in different words, and the rows arrive in the same eight-row precedence order one run's own block prints them in. **The units are turns, not minutes**: the median goes in the numeric column and the range beside it, punctuated exactly as every duration row on this page is. **A run nothing was read for is excluded from the sample rather than counted as eight zeroes** — the same `is_measured` distinction one run's block withholds its own rows on — while a measured run that issued no turn of a kind contributes a real zero, because that _is_ a reading. `--json` carries the same figures under `contributors`, so the [`diag`](https://github.com/joshuafolkken/kit/blob/main/.claude/skills/diag/SKILL.md) skill reads them without re-deriving anything. **`--period` reads the breakdown from `.time-history.jsonl`**, where [`josh followup`](#josh-followup) now writes it beside the figures it already recorded: the field is optional for the reason `started_at` is, so every line written before it exists carries none, is left out of each row's sample count, and is counted in the report's notes — a period whose records all predate it prints eight `not measured` rows and one sentence saying why, never a table of zeroes.
...
See [docs/eval.md](./eval.md) for when to run the suite by hand, the verdict meanings, and how a
scenario is judged.
