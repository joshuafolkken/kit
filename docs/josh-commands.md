...
Standard output carries one token per line — the runnable issues, in the order they may be started, or a single verdict word. Every explanation goes to standard error, so `answers=$(pnpm josh backlog:next)` captures something a loop can branch on. **A child of this repository is a bare number and a child elsewhere is written `owner/repo#N`**, because an epic may track a child in another repository and a bare number there names _this_ repository's issue of that number — a different issue entirely.
...
Followup stages (in run order):
  ...
  closes-check                0.6 s   1 invocation(s)
  context                     1.2 s   1 invocation(s)
  ...
  completion-comment          0.9 s   1 invocation(s)
  epic-close                  1.4 s   1 invocation(s)
