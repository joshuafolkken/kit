...
It reads the preview server's debug log — the path `WRANGLER_LOG_PATH` names, `e2e-web-server-logs` by default, a directory or a single file — and looks for wrangler's structural marker for a dead worker, `Error in ProxyController`. An assertion failure never produces it, so a suite that merely failed does not match. The verdict is written to `$GITHUB_OUTPUT` as `crashed`, and announced on the run as a notice.
...
**`Network connection lost.` is deliberately not a signature**, though it sits in the `cause` of every observed crash: workerd logs it for any aborted in-flight request, so matching it would hand a retry to exactly the failing suite this rule exists to expose.
