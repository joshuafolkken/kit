...
- **The two epic body sections**, `## Decisions` and `## Split rationale`, are matched whole and count when either names both issues anywhere in it. They hold many records with no delimiter a reader can rely on, so asking a stricter question would mean guessing where one record ends. **`## Split rationale` is read as well as `## Decisions`**, because `josh epic … --ordered` declares a whole chain and writes no `## Decisions` at all — a check that looked only there would report every correctly-documented ordered epic the moment it was created.
