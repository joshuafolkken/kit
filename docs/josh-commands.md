...
**For a lane started by [`josh lane:dispatch`](#josh-lanedispatch) the process trace is the deciding input, not a supplement** ([#1749](https://github.com/joshuafolkken/kit/issues/1749)). The child is an operating-system process of its own, so a log that is not moving is a session thinking rather than one that died — only `pgrep -laf` against the lane's directory tells the two apart. Run it and pass what it found; `--process alive` above is that answer in the ordinary case, never a constant to pass because the child was dispatched.
