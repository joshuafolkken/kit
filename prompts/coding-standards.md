...
- **仕組みを足す前に原因を問う**: これは**書かれたコードの可読性**の規則。仕組み（フック・ガード・ルール）を足すか・どこへ置くかという**設計時の判断**は `prompts/collaboration-workflow/simplicity-first.md` の担当なので、本文を写さずそちらを参照する
...
- **複雑度が高い場合**: 関数の認知的複雑度が4を超える、または行数（コード行数）が25行を超える場合は分割を検討
...
- **認知的複雑度**: 最大4
- **ネストレベル**: 最大2
- **関数の行数**: 最大25行（テストファイルは35行）
- **ファイルの行数**: 最大300行
- **関数内の文の数**: 最大10
- **パラメータ数**: 最大4個
- **行数はコード行数であり物理行数ではない**: `max-lines` / `max-lines-per-function` は `skipBlankLines` と `skipComments` を有効にして動くため、空行とコメントは数えない。`wc -l` ではなく `pnpm josh lint` の報告で判断する（joshuafolkken/kit#1070）
