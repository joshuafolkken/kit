...
ワークフロー内のレビュー工程（各フローの `/review`）は、**`pnpm josh git -y` で PR を作成した後にのみ**実行する — コミット前バリアントは存在せず、`fullrun` / `halfrun` / `queue` のすべてが同じ対象（PR ブランチの `git diff main...HEAD`）をレビューする。実行は kit が配布する `code-reviewer` サブエージェント（`.claude/agents/code-reviewer.md`）の起動による。実装したセッションとはコンテキストが分離されているため、実装時の思い込みがレビューに持ち込まれない。渡すのは diff の範囲と Issue のタイトル・目的だけであり、実装セッション側の説明や変更サマリは渡さない（作者のバイアスをレビューに再注入するため）。出力は `prompts/review.md` のテンプレートに従うので、下記のチェーンルール・決定表はそのまま適用される。**レビューは 1 ラウンド終わるたびに、その全文を独立した PR コメントとして投稿する**（`gh pr comment <branch> --body-file <path>`）— 投稿するのはそのラウンドのレビューが返った直後、指摘を修正する**前**である。各コメントの先頭は `## Code review — round N` とし、番号は 1 から始めて後続コミット後の再レビューでも連番を継続する。`N` は、その PR に既についている `## Code review — round` コメントの件数を数えて 1 を足して決める（`gh pr view <branch> --json comments --jq '[.comments[].body | select(test("^## Code review — round"))] | length'`）— 再開したセッションには前ラウンドの記憶がなく、1 から振り直すと「どれが確定版か」という、このルールが解消したはずの曖昧さが戻るため。中間ラウンドも**全文**を載せ、前ラウンドとの差分形式にはしない（差分だと後から単体で読んだときに意味が通らないため）。最終ラウンドには High/Medium がゼロで確定した旨を明記するので、どれが確定版かは曖昧にならない。**確定版 1 件だけを投稿する方式をこれで置き換える**：その方式ではレビューループ全体が最終ラウンドのタイムスタンプ 1 点に潰れ、各修正コミットを生んだ指摘そのものが失われるため、3 ラウンド回ったレビューと 1 ラウンドで終わったレビューを外から区別できなかった。コメントの投稿者は実行ユーザーの GitHub アカウントなので、`followup` の AI レビュアーブロッカー判定（`claude` / `coderabbitai` 著者）には該当しない — High/Medium を含む中間ラウンドを投稿してもマージ判定を妨げないのは、この一点による。サブエージェント機構のない AI ツール（Gemini / Cursor）では、代わりに新しいセッション／コンテキストで `prompts/review.md` のチェックリストを実行する。
...
| Findings, but all are Low               | Low only             | Post this round's review markdown as a PR comment, then immediately run `pnpm josh followup --merge` (Low findings may be skipped with a one-line reason per Pre-commit Self-Review)                                                                             |
| One or more High / Medium findings      | High and/or Medium   | **Post this round's full review markdown as a `## Code review — round N` PR comment first**, then fix in place, re-stage, commit, push, re-run `/review`, loop. Do NOT report the findings narratively to the user and wait. Do NOT call `followup --merge` yet. |
...
2. **Severity check** — Count high/medium findings across all categories. If ≥1 → post this round's review comment first, then fix in place, re-stage, commit, push, re-run `/review`. Loop until none remain. Do NOT call `followup --merge` yet.
...
**送信前セルフチェック（`/review` チェーン規則の自己チェックと同じ位置づけ）**: 報告を送る直前に次の 2 点を確認する。
