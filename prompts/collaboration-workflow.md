...
3. **新規 EPIC を作る前に、`#N` が既に EPIC の子でないかを確かめる** — `pnpm josh epic:bundle <N>`。「EPIC に属している」だけでなく、**どの EPIC かを名乗る**（`#893 already tracks this issue`）
   ...
   **`--add ... --before <N>` 自身が拒否されることもある。** EPIC が `#N` を名指さない順序を宣言している場合で、`#N` is not named in the declared dependency order と答えて何も書き込まない。本文を手で書き換えてはならず、2 つ目の EPIC も作らない。**コマンドの拒否がそのまま報告内容である。** この分岐はもともと停止で終わるので、拒否された内容を `confirmation` の Telegram に書いて人の判断に委ねる（joshuafolkken/kit#949）。
