...
3. **新規 EPIC を作る前に、`#N` が既に EPIC の子でないかを確かめる** — `pnpm josh epic:bundle <N>`。「EPIC に属している」だけでなく、**どの EPIC かを名乗る**（`#893 already tracks this issue`）
   ...
   | **コマンドが答えられなかった** — 非ゼロ終了、EPIC 一覧の取得失敗、`#N` が走査上限の外 | 何と言われたかを添えて停止・報告する。**EPIC 作成へ落ちてはならない。**「判定できなかった」は「どの EPIC も追跡していない」ではなく、そう読むとこの手順が防ごうとしている二重追跡が再発する                                                                                                                                     |
...
3. **作る前に `pnpm josh epic:bundle <N>` で、既に `#<N>` を追跡している EPIC が無いかを確かめる。** どの EPIC かを名乗る（`#893 already tracks this issue`）。`epicrun` に渡された素の Issue が、既に誰かの EPIC の子であることはありうる——`#893` の子である `#943` に対して `epicrun #943` と打つことを止めるものは無い——そこで 2 つ目の EPIC を作ると、どちらの自動クローズも他方を知らないまま動く。しかも**人が見ていない入口で**起きる。
   ...
   | **コマンドが答えられなかった**           | `#<N>` を `needs-decision` で park して報告する。「判定できなかった」は「どの EPIC も追跡していない」ではなく、そう読むとこの手順が防ごうとしている二重追跡が再発する |
