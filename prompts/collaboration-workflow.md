...
3. **`--ordered` を付けて新規 EPIC を作成する** — `pnpm josh epic "<title>" <P> <N> --ordered`。`#N` は成果物そのものなので、昇格ではなく子として残す。**`--ordered` は必須であって好みの問題ではない。** 付けなければ本文は `None — the children are independent` と書かれ、`blocked-by` も記録されないため、続く `epicrun` が前提 Issue より先に `#N` を返しうる。この規則が守ろうとしている順序そのものが失われる
...
3. **新規 EPIC を作成する。** `#<N>` は実装中の成果物そのものなので、この経路は常に「昇格せず子として残す」側に落ちる——分割判定の昇格・新規の分岐を、その都度選ぶわけではない。**どのコマンドを使うかは、何が見つかったかで決まる。** `--ordered` は引数の順序をそのまま依存の鎖にするためである。
