...
3. **対象パッケージのリポジトリに新しい Issue を作成する（確認なし）**: `gh issue create -R <owner>/<repo> --title "<root-cause title>" --body "<root cause and context>"`。根本原因・再現・期待結果を Step 1 のテンプレに沿って記載する。ここで「起票してよいか」を尋ねて停止してはならない
