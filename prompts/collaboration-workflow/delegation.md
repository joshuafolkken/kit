...
- `epic-child` の検証経路は親が GitHub 上の子の状態を読むことであって、実行単位が返す要約ではないこと。その読み取りが `CLOSED` 以外を一律に失敗と読むことではないこと（`needs-human-review` で止まった子は設計上 open で返る）。分岐そのものは各エントリの手順書側にある
...
- 下調べに独立した調査単位が複数要るときは、直列にではなく 1 ターンでまとめて（fan-out）起動すること（joshuafolkken/kit#1847）。これは §2h のバッチ化原則が `Agent` の起動へ及んだもので、独立性が条件（前の答えを要する問いだけは直列に残す）であること、離れて起動されるため `batch:guard` が拾えずフックでは強制しない（＝散文で持つ）理由も手順書側にある
