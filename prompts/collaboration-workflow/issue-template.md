...
### 振る舞い変更 Issue の宣言と 2 つの必須見出し（`## 発火点` / `## ベースライン`）

**AI の振る舞いを変えると主張する Issue は、`## 発火点` と `## ベースライン` の 2 見出しを必ず含める。** 散文の規則は「配って終わり」になりやすく、効いたかどうかがマージ後に読み直されないためである（joshuafolkken/kit#2178 は、人がたまたま測ったから見つかった）。

- **対象かどうかは本文の宣言から読み、判断させない。** 本文に `- 種別: 振る舞い変更` の行がある Issue だけが対象で、コードだけを変える Issue（`- 種別: コードのみ`、または宣言なし）には 2 見出しを課さない。宣言は `## 背景` の直下に置く
...
- **検査は `pnpm josh issue:lint <path>` が行う。** 対象判定・2 見出しの有無・発火点の配送可否・ベースラインが「コマンド + 値」かを、起票前の本文ファイルに対してまとめて検査する
