...
<AI の振る舞いを変える Issue のみ。その規則が破れる瞬間に最初に呼ばれるツールコール名を 1 つだけ書く（例 `Bash` / `Edit` / `Read` / `Write` / `AskUserQuestion`）。配送表に載らない名前は、フックで配送できない発火点として印字される>

## ベースライン

<AI の振る舞いを変える Issue のみ。測定コマンドと現在の値を「コマンド + 値」で書く。自然文の測定説明は再実行できないため受け付けない>

- `<測定コマンド>` → <現在の値>

## 再現

<AI の振る舞いを変える Issue のみ。現象を再現したコマンドとその実際の出力を「コマンド + 出力」で書く。自然文は再実行できないため不可。出力はフェンス（``` または ~~~）で囲む>
...
### 振る舞い変更 Issue の宣言と 3 つの必須見出し（`## 発火点` / `## ベースライン` / `## 再現`）

**AI の振る舞いを変えると主張する Issue は、`## 発火点` と `## ベースライン` と `## 再現` の 3 見出しを必ず含める。** 散文の規則は「配って終わり」になりやすく、効いたかどうかがマージ後に読み直されないためである（joshuafolkken/kit#2178）。読みだけを根拠にした起票（既に直った現象の起票・壊れたままの現象の誤読）を防ぐため、再現の実測も課す（joshuafolkken/kit#2353）。

- **対象かどうかは本文の宣言から読み、判断させない。** 本文に `- 種別: 振る舞い変更` の行がある Issue だけが対象で、コードだけを変える Issue（`- 種別: コードのみ`、または宣言なし）には 3 見出しを課さない。宣言は `## 背景` の直下に置く
