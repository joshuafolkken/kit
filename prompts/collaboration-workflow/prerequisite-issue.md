...
- 実行中に見つかる「別の作業」は 4 種類（別パッケージ起因の欠陥／分割／前提／単なる気づき）で、手順がそれぞれ異なること。4 つ目の「気づき」は実行中の Issue を何も変えないため、確認を取らずに起票してそのまま走り続ける唯一の経路であり、その手順は 2i 節にある（joshuafolkken/kit#1649）
