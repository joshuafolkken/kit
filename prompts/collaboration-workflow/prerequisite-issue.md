...
3. **新規 EPIC を作る前に、`#N` が既に EPIC の子でないかを確かめる** — `pnpm josh epic:bundle <N>`。「EPIC に属している」だけでなく、**どの EPIC かを名乗る**（`#893 already tracks this issue`）
   ...
   | **コマンドが答えられなかった** — 非ゼロ終了、または `Nothing to bundle.` の上に出た一覧打ち切りの ⚠ 警告。**`⚠ The epic listing …` で始まる打ち切り警告がその場合である**（`hit its …-epic cap` と `stopped at the …-issue page ceiling` の 2 通りがあり、どちらも epic 一覧が最後まで読めていないことを言う。joshuafolkken/kit#1067） — `#N` を追跡する EPIC が上限の外にあっただけかもしれない。`⚠ Could not read #N.` は関係の読み取りが 1 件失敗しただけで EPIC 所属について何も言っておらず、断定的な答えはそのまま有効である（joshuafolkken/kit#950） | 何と言われたかを添えて停止・報告する。**EPIC 作成へ落ちてはならない。**「判定できなかった」は「どの EPIC も追跡していない」ではなく、そう読むとこの手順が防ごうとしている二重追跡が再発する                                                                                                                                     |
