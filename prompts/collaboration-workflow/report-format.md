...
**実行証跡は「動くこと」の証拠であり、`pnpm josh gate` の緑ではない（joshuafolkken/kit#2246）**。gate（lint・型・スペル・ユニット）は「回帰していないこと」の証拠であって「動くこと」の証拠ではなく、完了判定はいまその緑だけで成立していた。実行可能な経路を触った変更では、実際に叩いたコマンドとその出力（UI ならスクリーンショット）をこの行に置く。触っていない変更（ドキュメントのみ等）では「なし」と理由を書く。
