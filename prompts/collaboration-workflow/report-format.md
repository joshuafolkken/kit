...
**境界の必須異常系はコマンドが突きつける。** 差分が HTTP・プロセス起動・ファイル IO の境界を跨ぐとき、その境界の異常系（非 200・タイムアウト・空レスポンス・不正 JSON・レート制限など）は第 2 段に載せる。跨いだ境界と必須異常系語彙は `pnpm josh cases <path...>` が変更パスから読んで印字するので、判断ではなくコマンドの答えになる。ユニットスイートは構造的にネットワークを遮断している（`scripts/test/test-network-guard.ts`）ため、境界の異常系はスイートで実行されず、**宣言の側で突きつける**しかない。
