...
`CLAUDE.md` は毎ターン全文が読み込まれる。したがって常駐に書ける量は有限で、`scripts/claude/workflow-skills.test.ts` の `RESIDENT_CEILING_BYTES` がその上限を固定している。**どの規則を常駐に残すかは、書き手の重要度判断ではなく 3 問の順序判定で決める。**

### 第 0 問（先に問う）

> **その規則の答えは、機械的に読める入力だけから計算できるか。**

**できるなら、規則は文書で届けない — コマンドが答える。** `pnpm josh oracle:list` が既存の決定オラクルを 20 件以上列挙しており、各行が「判断 / コマンド / 回答語彙 / 手順の単一ソース文書」を持つ。文書に残るのは「このコマンドに聞け」という導線 1 行だけで、判断の手順そのものは書かない。`scripts/rules/decision-oracle.ts` がその単一ソースである。
...
**そうであれば、規則はドライバの状態遷移に書き、散文には書かない。** `pnpm josh run:step <N>` が、実行の現在位置をイベントストリーム・キャリーレコード・Issue 状態の 3 つだけから計算し、次の 1 手を印字する（`scripts/run/run-step.ts` がその単一ソース、joshuafolkken/kit#2248）。第 0 問が**判断**を機械化したのに対し、この問いは**順序**を機械化する — 順序を散文に書けば、ドライバが計算する状態機械を読み手が毎ターン組み直す 2 つ目の手順書が並ぶだけで、読み取り量は下がらない。`fullrun.md` / `backlogrun.md` / `SKILL.md` が実行順を `run:step` へ指すだけで再説明しないのはこのためである。第 0 問と同じく、順序を決める規則はコマンド（ドライバ）が答え、文書は導線 1 行を張るだけである。
