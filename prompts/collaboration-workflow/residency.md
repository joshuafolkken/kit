...
- **バックログの WIP 上限（オープン 30 件）** — 起票はワークフローのキーワードを打っていないターンでも起きる（別パッケージ起因の起票、ワークフロー外のセルフレビューから出た後追い起票、会話中に記録したくなった気付き）。ワークフローの中でしか効かない上限は、バックログを最も速く増やしていた経路を数え落とす。常駐に残すのは件数の数え方・起票しない判断・実行が詰まる起票の除外・**割り込みの 3 条件**だけで、手順と数字を動かす条件は [`wip-cap.md`](./wip-cap.md) にある（joshuafolkken/kit#1469、#1518）。3 条件を常駐から落とすと「重大かどうか」が判断に戻り、ポインタを開かないターンでは上限が無条件に効いて発見が捨てられる — 発火点としての条件そのものが常駐側の本体である。**その 3 条件の予算は、実行が詰まる起票の例示列挙（前提 Issue・別パッケージ起因・分割の子と epic・`new` 入口）を常駐から降ろして捻出した** — `CLAUDE.md` は `RESIDENT_CEILING_BYTES` まで 105 バイトしか残っておらず、両方は載らなかった。降ろした列挙は [`wip-cap.md`](./wip-cap.md) 側でマーカーに固定してあり、**削除ではなく移動である**。判断を残すか例示を残すかの二択で判断を残したのは、例示が無くても「実行が詰まるか」は読めば分かるが、条件が無ければ「重大か」は読んでも分からないからである
