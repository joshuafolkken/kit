...
| **早すぎる進捗報告**（`.claude/skills/workflow-commands/backlogrun.md` → 「Progress while the run is quiet」） | `pnpm josh rule:guard` — 待つことだけが目的の `Bash`（全区間が `sleep`、または `sleep` に `echo` ／ `:` ／ `date` が並ぶだけ）。既に生きているタイマーがある回、またはその報告が間隔前に着地する回に拒否する | 待機タイマーを自前で張っていない ＝ 時計は `run:progress` が 1 本だけ持っている                         |
