...
- 判定の問い（既定は「分割しない」。単独でマージできる成果物が 2 つ以上あること **かつ** 1 回の検証ゲートで確認できる範囲を明確に超えること、の両方が要る。目安は変更ファイル約 10・変更行約 400。joshuafolkken/kit#1469）
- 既定を変えた理由（バックログの純増は到着側の問題であり、分割判定はその 3 つの製造元の 1 つだった）と、差分の大きさは「分割してよい下限」であって「分割すべき理由」ではないこと
