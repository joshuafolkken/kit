...
**#2164 / #2178 の成果は後退しない** — バッチガードにもレーンガードポリシーにも触れないので、子は拒否で殺されず、効かない助言の毎ターンの文脈費用も払わない。上限超過時は `read:files` が「Read ツールで 1 ターンにまとめて読め」と返しフォールバックでも読み取りは畳まれたまま。編集自体を畳むコマンドは置かない（native な複数編集を優先、上の却下案第 1 項）。詳細は `docs/josh-commands.md` →「`josh read:files`」。
