...
**joshuafolkken/kit#1436 took the re-measurement the first of these sections invited, and it came back the other way.** "The narrowing is real in scope and does not show in the wall clock" recorded round 1's duration against its input at `r = 0.05` over 9 runs, and said in so many words that a larger sample was exactly what would overturn it. It did: over **23 runs**, read from `pnpm josh time --last 30 --json` without `--top`, the same correlation is **`r = 0.55`** against changed lines — a rank correlation of **0.66**, and `r = 0.56` against changed files — which at that sample sits well outside the interval the 9-run reading carried. **The relation is real, and it is the rest of this section that decides the question.** It points where the retired coefficient pointed, for a different reason.
