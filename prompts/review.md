...
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Implementing — the review has not started                    | **The single check by name** — `pnpm josh lint`, `pnpm josh cspell:dot`, `pnpm josh test:unit`, or the project's own type check. Never the whole gate. (Once a gate _has_ run, its block header names each command, including the type check's, which is resolved per project) |
...
| The round-2 fix delta is in                                  | Join, and read what the gate printed before `pnpm josh bump minor`                                                                                                                                                                                                             |

**The criterion is the review, not the tree.** Once the gate has been started beside the review, every later edit — a red check's fix, a round-1 finding's fix — changes the tree and makes the last result stale, so rows 3 and 4 govern from there and a single check no longer answers for the commit. Reading the criterion as "no gate has run on _this exact_ tree" would send a run back to row 1 after every fix, which is the opposite of what the rows say.

**Nothing here weakens the join**, and the first row is the only one that is new. The gate the commit rests on is still the one started beside the review and read before the bump; what the first row removes is the probing in front of it, which proves nothing the join does not prove again a few minutes later.
