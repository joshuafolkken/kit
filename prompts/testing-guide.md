...
- **Use `*.test.ts` — never `*.spec.ts`.** The `.spec.ts` suffix is forbidden for unit/integration tests. This is **lint-enforced**: the generic test-filename rule building blocks (`eslint/rules/test-filename.js`, exported from kit as `@joshuafolkken/kit/eslint/test-filename`) are wired into whichever ESLint config consumes them and flag any `*.spec.ts` / `*.spec.js` file — and any file under a top-level `tests/` directory — and fail `josh lint`. (A vitest `{test,spec}` matcher would still accept both, but the lint rule fails loudly before a stray `.spec.ts` can drift in.)
...
> **Enforcement:** the `*.spec.ts` ban and the top-level `tests/` ban are enforced by the test-filename rule (`eslint/rules/test-filename.js`, exported from kit as `@joshuafolkken/kit/eslint/test-filename`), so they fail loudly in `josh lint` for every consumer whose ESLint config consumes the rule and every AI tool. This was chosen over tightening the vitest `{test,spec}` matchers to `{test}` only, because matcher-tightening causes **silent non-execution** of stray `*.spec.ts` files whereas a lint rule fails loudly. (Doc-only guidance had already failed to prevent this drift twice.)
