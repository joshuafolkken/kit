...
- **Use `*.test.ts` — never `*.spec.ts`.** The `.spec.ts` suffix is forbidden for unit/integration tests. This is **lint-enforced**: the shared SvelteKit ESLint config (`eslint/rules/test-filename.js`, wired into `create_sveltekit_config`) flags any `*.spec.ts` / `*.spec.js` file — and any file under a top-level `tests/` directory — and fails `josh lint`. (The `vite.config.ts` matchers still accept both `{test,spec}`, but the lint rule fails loudly before a stray `.spec.ts` can drift in.)
...
> **Enforcement:** the `*.spec.ts` ban and the top-level `tests/` ban are enforced by `eslint/rules/test-filename.js` (wired into `create_sveltekit_config`), so they fail loudly in `josh lint` for every consumer and every AI tool. This was chosen over tightening the `vite.config.ts` matchers to `{test}` only, because matcher-tightening causes **silent non-execution** of stray `*.spec.ts` files whereas a lint rule fails loudly. (Doc-only guidance had already failed to prevent this drift twice.)
