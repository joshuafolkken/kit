import { backlog_epic_loop, BACKLOG_EPIC_SKILL } from '#scripts/backlog-epic-loop-fixture'
...
describe('the per-child loop slice', () => {
	it('is bounded by the step that follows it', () => {
		const step = backlog_epic_loop.per_child_step()
	...
	it('throws when the start marker is gone', () => {
		expect(() => backlog_epic_loop.slice_between('no loop here', 'start', 'end')).toThrow(
			BACKLOG_EPIC_SKILL,
		)
	})
	...
	it('throws when only the end marker is gone', () => {
		expect(() =>
			backlog_epic_loop.slice_between('a start and nothing after', 'start', 'end'),
		).toThrow(BACKLOG_EPIC_SKILL)
	})
	...
	it('names the missing marker', () => {
		expect(() => backlog_epic_loop.slice_between('', 'absent marker', 'also absent')).toThrow(
			'"absent marker"',
		)
	})
})
