...
function index_of_marker(content: string, marker: string): number {
	...
	if (found === -1) {
		throw new Error(`${BACKLOG_EPIC_SKILL} no longer contains ${JSON.stringify(marker)}`)
	}
...
function per_child_step(): string {
	return slice_between(read_repo_file(BACKLOG_EPIC_SKILL), LOOP_START, NEXT_STEP)
}

const backlog_epic_loop = { per_child_step, slice_between }

export { backlog_epic_loop, BACKLOG_EPIC_SKILL }
