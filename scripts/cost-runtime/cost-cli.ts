...
// `josh cost` — what the next turn of a run will cost, read from Claude Code's own session
// transcripts (joshuafolkken/kit#962).
//
...
// hand-off aggregates the 300,000 experiment is judged against.

const ARGV_OFFSET = 2
const FAILURE_EXIT_CODE = 1
const USAGE = 'Usage: josh cost --over <tokens-per-request> [--path <dir>]'
...
function run(argv: ReadonlyArray<string>, cwd: string = process.cwd()): number {
	...
	return run_over(transcript_cwd.resolve(options.path, cwd), options.over)
}
