...
// branch tree alone did not. joshuafolkken/kit#2181 raised it two further slack units (108,160 →
// 109,184) for the mandated `josh disposition` section — the runtime-path oracle sharing the
// review-level inert set, its command reference and its `runtime` / `non-runtime` verdict — and the
// `josh test:declared --match` note, landing against a document already at its mark, the branch's merge
// with `main` layering the additions on top of #2218's, so the merged tree needs two units the branch
// tree alone did not.
...
const DOCUMENT_BYTE_BUDGET: ReadonlyArray<DocumentBudget> = [
	...
	{ path: 'docs/josh-commands.md', bytes: 108_189 },
