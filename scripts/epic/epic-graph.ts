...
// made `epic_cross_repo.resolve_cross_repo`'s cross-repository branch unreachable: every blocker
// arrived carrying the blocked child's repository, so `blocker.repo === blocked.repo` always held.
interface EpicChild {
	number: number
	repo: string
	state: 'OPEN' | 'CLOSED'
...
function blockers_of(index: ReadonlyMap<string, EpicChild>, node: string): ReadonlyArray<string> {
	...
	return child.blocked_by.map((blocker) => blocker_key(blocker))
}
...
function is_link_recorded(
	...
	const blocker = reference_key(declared_repo, link.blocker)

	return children.some(
		(child) =>
			child.number === link.blocked &&
			child.repo === declared_repo &&
			child.blocked_by.some((recorded) => key_of(recorded) === blocker),
	)
}
...
function child_links(
	...
	if (child.repo !== declared_repo) return []

	return child.blocked_by
		.filter((blocker) => blocker.repo === declared_repo && keys.has(key_of(blocker)))
		.map((blocker) => ({ blocker: blocker.number, blocked: child.number }))
}
...
function undeclared_relations(
	...
	const keys = new Set(children.map((child) => key_of(child)))

	return children
		.flatMap((child) => child_links(child, keys, declared_repo))
...
function find_anomalies(
	...
	const missing = missing_relations(links, children, declared_repo)
	const undeclared = undeclared_relations(links, children, declared_repo)
