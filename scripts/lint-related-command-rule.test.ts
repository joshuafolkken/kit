...
// joshuafolkken/kit#1298: the scoped lint check only saves anything if a run reaches for it, and a
// run reaches for whichever command the implementation-loop row names. Registration and that row
// are therefore asserted together — a command nothing points at is the whole repository read again
// on every re-check, and a row pointing at a command that does not exist is a failed re-check.

const COMMAND_NAME = 'lint:related'
const SCRIPT_PATH = 'scripts/lint-related.ts'
const ALIAS = 'lr'

// The whole tree is what the gate reads before the commit. A document that swapped this one for the
// scoped command would narrow the check a merge rests on, which is the inversion this whole change
// is written not to make.
const WHOLE_TREE_COMMAND = 'pnpm josh lint'
const SCOPED_COMMAND = 'pnpm josh lint:related'
...
const IMPLEMENTATION_LOOP_DOCUMENTS: ReadonlyArray<string> = [
	...
	// The re-run list here names the same two scoped checks, and a document nobody asserts is where
	// the migration silently left one behind.
...
describe('the implementation loop names the scoped lint check', () => {
...
// by, that the gate is unchanged, and that an unreadable change list checks everything rather than
// nothing.
const REVIEW_PROMPT_MARKERS: ReadonlyArray<string> = [
	'**Two of the checks in the first row are scoped, and neither replaces what the gate runs.**',
	'falls back to the whole tree',
	`\`${WHOLE_TREE_COMMAND}\` is unchanged as what \`pnpm josh gate\` runs`,
]
...
const COMMAND_DOC_MARKERS: ReadonlyArray<string> = [
	`### \`josh ${COMMAND_NAME}\``,
	'**It is added in front of the whole tree, never in place of it.**',
	'**It prints what it narrowed by before it runs**',
	'the changed files could not be read',
	'no changed file is one prettier or eslint reads',
]
