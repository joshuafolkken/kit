...
// pnpm 11+ ignores package.json's `pnpm.overrides`; only workspace overrides affect resolution.
// Keep reading the old field for diagnostics, but never treat it as an effective override.
function read_overrides(sources: OverridesSources): Record<string, string> {
	return read_overrides_from_workspace(sources.workspace_yaml)
}
...
function describe_sources(sources: OverridesSources): string {
	const counts = [
		...
		...count_entry(PACKAGE_JSON_PATH, read_overrides_from_package(sources.package_json)).map(
			(entry) => `${entry} (ignored by pnpm)`,
		),
	]
