...
describe('DELIVERED_RULES — the enumeration', () => {
	...
	it.each([WIP_CAP, ISSUE_COMMENTS])('names %j', (id) => {
		expect(delivered_rules.DELIVERED_RULES.map((rule) => rule.id)).toContain(id)
	})

	it.each([FILING_COMMAND, FILING_API_COMMAND, BODY_READ_COMMAND, BODY_READ_API_COMMAND])(
		'is claimed by exactly one rule: %j',
		(command) => {
