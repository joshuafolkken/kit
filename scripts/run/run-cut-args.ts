...
const USAGE =
	'Usage: josh run:cut <issue> [--impl] [--handoff <path>] | --resume <issue> | --json | --end'
...
// the caller has to remember. `--impl` and `--handoff` are not modes — they modify the bare cut — so
// each is refused only when it accompanies one of the three that ask about a cut rather than take one.
// `--handoff` pairs freely with `--impl`: it carries the instruction the cut records.
function is_single_mode(values: ParsedValues['values']): boolean {
	...
	const has_cut_modifier = values.impl === true || values.handoff !== undefined

	return !(has_cut_modifier && modes.length > 0)
}

// What a cut carries beyond its issue: whether `--impl` named the implementation boundary rather than
// the pre-gate one, and the `--handoff` path for the instruction it carries. A resume needs none of it.
interface CutExtras {
	is_implementation: boolean
	handoff_path?: string | undefined
}
