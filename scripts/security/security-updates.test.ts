...
describe('report_security_updates_section', () => {
	it('prints the enabled status for a resolvable repository', () => {
		stub_gh(OK, ENABLED_BODY)

		expect(security_updates.report_security_updates_section(REPO)).toBe('enabled')
	...
	it('never spawns gh when the caller supplies an unresolved repository', () => {
		...
		expect(security_updates.report_security_updates_section(undefined)).toBe('unreadable')
		expect(resolve).not.toHaveBeenCalled()
	...
	it('reports unreadable for a supplied repository whose setting cannot be read', () => {
		...
		expect(security_updates.report_security_updates_section(REPO)).toBe('unreadable')
	})
