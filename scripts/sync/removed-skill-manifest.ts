...
// measurement; this removes the copy a consumer received before then (joshuafolkken/kit#1990).
// `removed-skill-manifest.test.ts` recomputes these from the repo's own skill source and fails on
// drift, so the frozen hashes can never go stale silently.
