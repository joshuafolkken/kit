...
function is_covered(inner: Interval, covered: ReadonlyArray<Interval>): boolean {
	return covered.some((one) => encloses(one, inner))
}
...
function inside_window(
	...
	return spans.filter((span) => {
		...
		return encloses(window, one) && !overlaps(one, outside)
	})
}
...
function unit_windows(
	...
	read: SpanReader,
): { inside: Array<Interval>; outside: Array<Interval> } {
	const found = units.flatMap((unit) => {
		const window = window_of(read(unit) ?? [])

		return window === undefined ? [] : [{ unit, window }]
	})
