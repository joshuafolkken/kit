...
// Units built from hand-made records rather than a corpus walk: there is no transcript to read a
// purpose from, so they come back `unknown` — the classifier's own suite covers the reading itself.
const NO_CORPUS: Corpus = {
	sessions: [],
	files: [],
	missing: {
		no_usage_lines: 0,
...
describe('time_request_costs.delegated_units — pricing the construction request', () => {
	...
	it('prices the construction request, not the first record', () => {
		const units = time_request_costs.delegated_units(
			[delegated('agent-a', 10, 500_000), delegated('agent-a', 500_000, 500_000)],
			NO_CORPUS,
		)
		...
		// The unit's model is read from its construction request.
		expect(units[0]?.model).toBe(OPUS)
	})
	...
	it('marks a unit on a model the table does not know unpriced', () => {
		...
		expect(time_request_costs.delegated_units([bare], NO_CORPUS)[0]).toMatchObject({
			is_priced: false,
			cost_usd: 0,
		})
	})
...
describe('time_request_costs.delegated_units — reading model and purpose', () => {
	// The purpose is read from the unit's own transcript, found by session id among the corpus files.
	it('reads a unit purpose from its transcript', () => {
		const directory = mkdtempSync(path.join(tmpdir(), 'unit-purpose-'))
		const transcript_path = path.join(directory, 'agent-c.jsonl')

		writeFileSync(transcript_path, 'the fork ran pnpm josh review:brief on the diff')
		...
		expect(units[0]?.purpose).toBe('review')
	})
})
...
describe('time_request_costs.delegated_units — what it excludes', () => {
	it('ignores a main-line session', () => {
		...
		expect(time_request_costs.delegated_units([main], NO_CORPUS)).toEqual([])
	})
})
